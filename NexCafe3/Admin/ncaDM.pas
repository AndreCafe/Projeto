...
begin
  if NumAberto<1 then
    Raise Exception.Create('O caixa está fechado. É necessário abrir o caixa para executar essa operação.');
end;  
...
begin
  ...
  if mtMaquina.Locate('ObjectInt', Integer(Sender), []) then begin
    FI  := FrmPri.FM.FormByClass(TfbMaquinas);
    if (FI<>nil) and (FI^.fiInstance<>nil) then
      TfbMaquinas(FI^.fiInstance).DeleteMaq(TncMaquina(Sender));
    mtMaquina.Delete;
  end; 
end;
