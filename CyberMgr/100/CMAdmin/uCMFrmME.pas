...
uses
  ...
  lmdgroup, lmddbctA, ffdb, dxGrClEx, 
...
type
  ...
  TFrmME = class(TForm)
    ...
    panCab: TLMDSimplePanel;
    Label1: TLabel;
    Label2: TLabel;
    OvcController1: TOvcController;
    Label3: TLabel;
    cbTipo: TDBComboBox;
    edSerie: TOvcDbSimpleField;
    edNumero: TOvcDbNumericField;
    ...
    procedure cmConfirmaClick(Sender: TObject);
...
procedure TFrmME.cmConfirmaClick(Sender: TObject);
var St: String;
begin
  if tMEEntrada.Value then 
    St := 'Entrada'
    ...
    St := 'Saída';  
  if MessageDlg('Confirma realmente a ' + St + ' em ' + 
                FormatDateTime('dd/mm/yyyy hh:mm', Now) + ' ? ', 
                mtConfirmation, [mbYes, mbNo], 0) = mrYes then
  begin       
    if not (tME.State in [dsEdit, dsInsert]) then tME.Edit;
    tMEDtMovimentacao.Value := Now;
    cmGravarClick(cmGravar);
  end;  
end;
