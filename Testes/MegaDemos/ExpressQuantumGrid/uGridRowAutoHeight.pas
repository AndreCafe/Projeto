...
type
  TfrmGridAutoHeight = class(TdxGridFrame)
    ...
    DBTableViewCAPTION: TcxGridDBColumn;
    DBTableViewPLOTOUTLINE: TcxGridDBColumn;
...
begin
  if AItem = DBTableViewPHOTO then
    if not TcxImageProperties(DBTableViewPHOTO.Properties).Stretch then
    begin
...
procedure TfrmGridAutoHeight.edImageHeightPropertiesChange(
  Sender: TObject);
begin
