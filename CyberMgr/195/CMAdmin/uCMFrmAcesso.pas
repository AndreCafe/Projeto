...
uses
  ...
  OvcDbNF, OvcSF, OvcDbSF, OvcPF, Buttons, ImgList, dxEdLib, dxInRwEx,
  dxLayout;
...
type
  ...
  TFrmAcesso = class(TForm)
    ...
    InspCad: TdxInspectorDBExtLookupRow;
    TabDebitoCli: TCurrencyField;
    InspCabDeb: TdxInspectorDBCurrencyRow;
    ...
    procedure InspCadCloseUp(Sender: TObject; var Text: String;
      var Accept: Boolean);
    procedure InspCadValidate(Sender: TObject; var ErrorText: String;
      var Accept: Boolean);
    procedure InspCabChangeNode(Sender: TObject; OldNode,
      Node: TdxInspectorNode);
    procedure InspCabExit(Sender: TObject);
    procedure InspCabDebDrawValue(Sender: TdxInspectorRow;
      ACanvas: TCanvas; ARect: TRect; var AText: String; AFont: TFont;
      var AColor: TColor; var ADone: Boolean);
...
uses uCMFrmPri, uCMDados, cmIDRecursos, cmAuxPassaporte;
...
begin
  ...
  if InspPagto.Visible and Dados.tbConfigImprimirImpRec.Value then
    cmRecibo.Visible := ivAlways;

  if InspPagto.Visible and (not Permitido(daAlterarSinal)) then
...
procedure TFrmAcesso.InspCadValidate(Sender: TObject;
...
begin
  ...
  InspCabDeb.Visible := (TabDebitoCli.Value>0.009);
end;

procedure TFrmAcesso.InspCabChangeNode(Sender: TObject; OldNode,
  Node: TdxInspectorNode);
begin
  MostraDebito;
end;
...
procedure TFrmAcesso.InspCabDebDrawValue(Sender: TdxInspectorRow;
  ACanvas: TCanvas; ARect: TRect; var AText: String; AFont: TFont;
  var AColor: TColor; var ADone: Boolean);
begin
  AFont.Style := [fsBold];
