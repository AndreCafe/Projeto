...
uses
  ...
  cxImage, cxDBEdit, cxRadioGroup;

type

  TFrmAcesso2 = class(TForm)
    ...
    InspCabCliCad: TcxDBMultiEditorRow;
...
var
  FrmAcesso2: TFrmAcesso2;

implementation

uses ucmaPri, ucmaDM, cmIDRecursos, cmAuxPassaporte, ufmImagens, ucmaFrmCliente;

{$R *.DFM}

procedure TFrmAcesso2.GravaIniciar;
begin
...
function TFrmAcesso2.GetSinal: Double;
begin
  try
...
procedure TFrmAcesso2.GravaEditar;
var
  M : TCMMaquina;
      ...
      try
        ...
        if M <> nil then begin
          ...
          if M.AguardaPagto then
            ...
            LePassaportesCliTran(Dados.tbPass, Dados.tbHistPass, M.TipoAcesso, M.Contato, M.Acesso, M.Passaportes);
...
procedure TFrmAcesso2.cmGravarClick(Sender: TObject);
begin
  Edit1.SetFocus;
  MostraDebito;
  if FIniciar then
...
procedure TFrmAcesso2.cmCancelarClick(Sender: TObject);
begin
  Close;
end;

procedure TFrmAcesso2.cmEditarClienteClick(Sender: TObject);
begin
...
procedure TFrmAcesso2.EditarEsp(aTab: TnxTable; Pagina: Integer);
var 
  I : Integer;
  M : TCMMaquina;
begin
...
procedure TFrmAcesso2.Editar(aTab: TnxTable; Pagina: Integer);
begin
  EditarEsp(aTab, Pagina);
end;

procedure TFrmAcesso2.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
{  case Key of
    Key_Enter, Key_Tab : 
...
procedure TFrmAcesso2.TabCalcFields(DataSet: TDataSet);
begin
  RecalcValores;
...
procedure TFrmAcesso2.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Dados.tbCli.IndexName := 'ICodigo';
  Dados.tbPro.IndexName := 'ICodigo';
  Action := caFree;
...
procedure TFrmAcesso2.ResetaValores;
begin
...
procedure TFrmAcesso2.FormCreate(Sender: TObject);
begin
  FStream := TMemoryStream.Create;
  FIniciar := False;
  with Dados, InspCabCliCad.Properties do 
  case tbConfigCampoLocalizaCli.Value of
    ...
    Editors[0].Free;
    Editors[0].Free; 
    Editors[0].Caption := 'Cliente Cadastrado';
  end;  
...
procedure TFrmAcesso2.FormDestroy(Sender: TObject);
begin
  FStream.Free;
end;

procedure TFrmAcesso2.InspNomeContatoChange(Sender: TObject);
begin
  TabNome.Value := TabNomeContato.Value;
end;

procedure TFrmAcesso2.RecalcValores;
var
...
procedure TFrmAcesso2.RefreshFoto;
var S: String;
begin
  with Dados do
...
procedure TFrmAcesso2.TabQtdPretoChange(Sender: TField);
begin
  RecalcValores;
end;

procedure TFrmAcesso2.InspRow12CloseUp(Sender: TObject; var Value: Variant;
  var Accept: Boolean);
begin
  Accept := True;
...
function TFrmAcesso2.SalvarItem: Boolean;
begin
  Result := 
...
procedure TFrmAcesso2.LeSinais;
begin
  mtSinal.EmptyTable;
...
procedure TFrmAcesso2.SalvaSinais;
var 
...
procedure TFrmAcesso2.SalvaItens;
var 
  I : Integer;
  Data : TDateTime;
  Produto : String;
...
procedure TFrmAcesso2.mtItensAfterInsert(DataSet: TDataSet);
begin
  mtItensItem.Value := mtItens.RecordCount+1;
  mtItensQuant.Value := 1;
end;

procedure TFrmAcesso2.mtItensAfterDelete(DataSet: TDataSet);
begin
  if mtItens.RecordCount=0 then 
    mtItens.Append;
end;

procedure TFrmAcesso2.tMovEstCalcFields(DataSet: TDataSet);
begin
...
procedure TFrmAcesso2.FormShow(Sender: TObject);
begin
...
procedure TFrmAcesso2.mtItensBeforeDelete(DataSet: TDataSet);
begin
...
function TFrmAcesso2.ItemAtual: Integer;
begin
  Result := Succ(TVProd.DataController.FocusedRecordIndex);
end;

procedure TFrmAcesso2.lbVendasClick(Sender: TObject);
begin
  GridProd.SetFocus;
end;

procedure TFrmAcesso2.mtItensProdutoValidate(Sender: TField);
var V: Variant;
begin
...
procedure TFrmAcesso2.mtItensCalcFields(DataSet: TDataSet);
begin
  FTotVendas := FTotVendas - mtItensTotal.Value;
...
procedure TFrmAcesso2.mtSinalBeforeDelete(DataSet: TDataSet);
begin
...
procedure TFrmAcesso2.mtSinalCalcFields(DataSet: TDataSet);
begin
...
procedure TFrmAcesso2.cmNovoSinalClick(Sender: TObject);
begin
  mtSinal.Append;
...
procedure TFrmAcesso2.edPausaTempoChange(Sender: TObject);
begin
...
procedure TFrmAcesso2.edPausaHoraChange(Sender: TObject);
begin
  if Calculando then Exit;
  Calculando := True;
  try
...
procedure TFrmAcesso2.btnRecalcClick(Sender: TObject);
begin
...
procedure TFrmAcesso2.edSinalChange(Sender: TObject);
var 
  NumTicks: Cardinal;
  TA : TCMTipoAcesso;
begin
...
procedure TFrmAcesso2.edSinalPropertiesChange(Sender: TObject);
begin
  edSinal.PostEditValue;
end;

procedure TFrmAcesso2.edLimiteTempoChange(Sender: TObject);
var 
  NumTicks: Cardinal;
  TA : TCMTipoAcesso;
...
procedure TFrmAcesso2.TabAfterEdit(DataSet: TDataSet);
begin
  FTipoAnt  := TabTipoAcesso.Value;
end;

procedure TFrmAcesso2.InspCadEditButtonClick(Sender: TObject);
begin
  Dados.AjustaCampoLocalizaCli;
end;

procedure TFrmAcesso2.cmManutencaoClick(Sender: TObject);
begin
  with Dados, CM do 
...
procedure TFrmAcesso2.InspCadCloseUp(Sender: TObject; var Text: String;
  var Accept: Boolean);
begin
  Accept := True;
  MostraDebito;
...
procedure TFrmAcesso2.InspDescontoPropertiesGetEditingProperties(
  Sender: TcxCustomEditorRowProperties; ARecordIndex: Integer;
  var AProperties: TcxCustomEditProperties);
begin
...
procedure TFrmAcesso2.MostraDebito;
begin
...
procedure TFrmAcesso2.cmReciboClick(Sender: TObject);
var
  Arq: TextFile;
  I : Integer;
  SL : TStrings;
...
procedure TFrmAcesso2.InspCabItemChanged(Sender: TObject;
  AOldRow: TcxCustomRow; AOldCellIndex: Integer);
begin
  MostraDebito;
end;

procedure TFrmAcesso2.InspCabExit(Sender: TObject);
begin
  MostraDebito;
end;

procedure TFrmAcesso2.InspEnter(Sender: TObject);
begin
  Insp.FocusedRow := InspDesconto;
end;

procedure TFrmAcesso2.TVSinalEditing(
  Sender: TcxCustomGridTableView; AItem: TcxCustomGridTableItem;
  var AAllow: Boolean);
var V : Variant;  
begin
...
procedure TFrmAcesso2.TVProdFocusedRecordChanged(
...
procedure TFrmAcesso2.GridProdEnter(Sender: TObject);
begin
  TVProd.Controller.FocusedItem := TVProdProduto;
end;

procedure TFrmAcesso2.TVProdMouseDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
...
procedure TFrmAcesso2.TVProdUnitarioGetProperties(Sender: TcxCustomGridTableItem;
  ARecord: TcxCustomGridRecord; var AProperties: TcxCustomEditProperties);
begin
  AProperties.ReadOnly := not mtItensPodeAlterarPreco.Value;
end;

procedure TFrmAcesso2.TVProdKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  case Key of
    Key_Del : 
...
procedure TFrmAcesso2.TVProdCustomDrawFooterCell(Sender: TcxGridTableView;
  ACanvas: TcxCanvas; AViewInfo: TcxGridColumnHeaderViewInfo;
  var ADone: Boolean);
begin
...
procedure TFrmAcesso2.InspCabCliCadEditPropertiesInitPopup(Sender: TObject);
begin
  Dados.tbCli.IndexName := 'INome';
end;

procedure TFrmAcesso2.cmNovoItemClick(Sender: TObject);
begin
  mtItens.Append;
end;

procedure TFrmAcesso2.cmRemoverItemClick(Sender: TObject);
begin
  mtItens.Delete;
end;

procedure TFrmAcesso2.InspCabAvulsoEditPropertiesEditValueChanged(
  Sender: TObject);
begin
  RefreshFoto;
end;

procedure TFrmAcesso2.InspCabCliCadEditors0EditPropertiesInitPopup(
  Sender: TObject);
begin
  Dados.tbCli.IndexName := 'ICodigo';
end;

procedure TFrmAcesso2.InspCabCliCadEditors0GetEditingProperties(
  Sender: TcxCustomEditorRowProperties; ARecordIndex: Integer;
  var AProperties: TcxCustomEditProperties);
begin
...
procedure TFrmAcesso2.InspCabCliCadEditors1EditPropertiesInitPopup(
  Sender: TObject);
begin
  Dados.tbCli.IndexName := 'IUsername';
end;

procedure TFrmAcesso2.InspCabCliCadEditors2EditPropertiesCloseUp(
  Sender: TObject);
begin
  RefreshFoto;
  MostraDebito;
...
procedure TFrmAcesso2.InspCabCliCadEditors2EditPropertiesInitPopup(
  Sender: TObject);
begin
  Dados.tbCli.IndexName := 'INome';
end;
