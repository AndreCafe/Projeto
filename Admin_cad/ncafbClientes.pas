...
uses 
  ...
  ncSessao, ncDebTempo, ncaFrmDebTempo;
...
begin
  ...
  STotCred := 'Créditos = '+MinutosToHMSStr(Dados.TotalCreditos);
  if cmMostrar.ItemIndex=1 then begin
    ...
    if cmMostrar.ItemIndex=2 then FiltroBas := '(Minutos > 0)'; 
...
begin
  ...
  try
    ...
    Tempo := 0;
    Obs := '';
    if TFrmDebTempo.Create(nil).Editar(True, TabNome.Value, Tempo, Obs) then begin
      T.Obs := Obs;
      T.QtdTempo := Tempo;
