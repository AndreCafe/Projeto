...
uses
  ...
  dxNavBarBase, dxNavBar, cxSpinEdit, cxLookAndFeels;
...
begin
  ...
  if gConfig.PedirSaldoF then begin
    ...
    if not SimNao('Deseja realmente fechar o caixa atual?') then Exit;

  Dados.CM.FecharCaixa(NC, aSaldo);

  with Dados do 
  if (aObs>'') or (aIDLoja<>'') and tbCaixa.Locate('ID', NC, []) then begin
    tbCaixa.Edit;
    tbCaixaIDLivre.Value := aIDLoja;
    tbCaixaObs.Value := aObs;
    tbCaixa.Post;
  end;
    ...
    try      
      ...
      case Operacao of
        ftran_Cliente : begin
          ...
          tAP.Active := False;
          AjustaVisAP;
          TV.OptionsView.NoDataToDisplayInfoText := 'Não existem transações para esse cliente';
        end;
        ...
        ftran_Caixa : begin
          ...
          if FiltroF=Null then begin
            ...
            tAP.Active := False;
          AjustaVisAP;
          TV.OptionsView.NoDataToDisplayInfoText := 'Não foi realizada nenhuma operação nesse caixa';
        end
      else
        begin
          cmPeriodo.Visible := ivNever;
          ...
          if NumAberto>0 then begin
            ...
            TV.OptionsView.NoDataToDisplayInfoText := 'Ainda não foi realizada nenhuma transação nesse caixa';
            ...
            Tab.SetRange([-2], [-2]);
            TV.OptionsView.NoDataToDisplayInfoText := 'O caixa está fechado';
...
var 
  A, F: TDateTime; aObs, aIDLoja: String; aSaldoF : Currency;
begin
  inherited;
  A := DiaAberto;
  F := 0;
  ...
  aSaldoF := 0;
  aIDLoja := '';
  if TFrmCorrigeDataCaixa.Create(nil).Editar(NumAberto, A, F, aSaldoF, aObs, aIDLoja) then begin
...
begin
  ...
  try
    if not tbCaixa.Locate('ID', NumAberto, []) then
      Raise ENexCafe.Create('Caixa atual não encontrado');
