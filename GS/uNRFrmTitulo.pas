...
uses
  ...
  cxClasses, cxLookAndFeels, cxLookAndFeelPainters;
...
type
  TFrmTitulo = class(TForm)
    ...
    DS: TDataSource;
    MTSituacaoCobr: TWordField;
    VGSituacaoCobr: TcxDBEditorRow;
  ...
  public
    ...
    function Editar(AContato: Variant; ADataset: TDataset; aBaixar: TDateTime = 0): Boolean;
    { Public declarations }
  end;
...
function TFrmTitulo.Editar(AContato: Variant; ADataset: TDataset; aBaixar: TDateTime = 0): Boolean;
begin
  FNovo := False;
  FContato := AContato;
  FDataset := ADataset;
  ...
  MT.Post;
  MT.Edit;
  if aBaixar>0 then begin
    mtValorPago.Value := mtValor.Value;
    mtPagamento.Value := aBaixar;
  end;
  ShowModal;
  Result := FRes;
end;
