...
type  
  ...
  TArrayPausas = Array of TCMPausa;

  TCMPausas = class
  private 
    FPausas : TArrayPausas;
    function GetQuantidade: Integer;
    function GetItem(Index: Integer): PCMPausa;
  ...
  public
    ...
    property Itens[Index: Integer]: PCMPausa
      read GetItem;
  ...
  TCMTarifador = class ( TCMTarifadorBase )
    FInicio      : TDateTime;
    FAgora       : TDateTime;
    FNumTicks    : Cardinal;
    FTicksR      : Cardinal;
  ...
  protected  
    ...
    function GetCores: TCMListaCorPreco; override;
    function GetCredito: Cardinal; override;
    function GetHoraCor: PCMHoraCor; override;
    ...
    procedure SetCores(const Value: TCMListaCorPreco); override;
    procedure SetCredito(const Value: Cardinal); override;
    procedure SetHoraCor(const Value: PCMHoraCor); override;
...
procedure TCMTarifador.AtualizaDiaH;
...
procedure TCMTarifador.AvancaProxHora;
var Hour, Min, Sec, MSec : Word;
begin
  DecodeTime(FAgora, Hour, Min, Sec, MSec);
...
function TCMTarifador.CalculaEtapaTarifacao(aTempo, aTol: Cardinal; aPreco: Double): Boolean;
var
  RestoI: Integer;
  TempoR,
  TempoP : Cardinal;
...
procedure TCMTarifador.SetNumTicks(const Value: Cardinal);
begin
...
function TCMTarifador.Calcular(aTempoCalc: Cardinal): Boolean;
var 
  T: Cardinal;
  Avancou : Byte;
...
begin
  ...
  { 1a coisa é obter a Cor para a hora que estamos calculando. FHoraCor é uma matriz
    que contém o código de cor a ser usado para cada hora e dia da semana.
      
    O código de cor é guardado em FCorAI e é a chave para obter o objeto TCMCorPreco que
    contém os detalhes necessários para a tarifação. FCor e FCorA guardam a referencia
    para o TCmCorPreco. FCorA é um cache }
            
  if (FCorA<>nil) and (FCorAI=FHoraCor[FDia, FHora]) then
    FCor := FCorA
    { o código de cor atual é o mesmo do anterior então usamos o cache }
  else begin 

    { Mudou o código de cor temos que obter o objeto da lista de cores }
      
    FCorAI := FHoraCor[FDia, FHora];
    FCor := FCores.PorCor[FTipoAcesso, FCorAI];

    { Se FCor=nil significa que não existe nenhuma Cor associada ao horário que estamos,
      portanto vamos avançando no tempo até encontrar uma hora seguinte q tenha 
      uma cor associada. Mas pode ser que a tabela toda não tenha nenhuma cor associada, 
      nesse caso temos que abortar a Tarifação para não entrar em Loop Infinito }
         
    Avancou := 0;
    while (FCor=nil) and (Avancou<24) do begin
      AvancaProxHora;
      FCorAI := FHoraCor[FDia, FHora];
      FCor := FCores.PorCor[FTipoAcesso, FCorAI];
      Inc(Avancou);
    ...
    if (FCor=nil) {and (Avancou>=24)} then 
      Exit;
      // Rodamos 24H e não encontramos nenhuma cor então desistimos da tarifação
  end;  
    
  { Uma cor pode ter várias etapas de tarifação, em qual estamos ? }
  if (FCor<>FCorA) then begin
    ...
    TempoCorrido := aTempoCalc - FTicksCor;
    if (TempoCorrido>0) and (FCorA<>nil) and (FCorA.Reinicia) then
      { Só nos interessa o Tempo Corrido desde o ultimo inicio de 1a ETAPA da Cor Anterior }
      TempoCorrido := TempoCorrido mod FCorA.TempoTotal;

    { Obtem o indice da etapa de preço atual }
    FEtapa := FCor.ObtemIndProxPreco(TempoCorrido);
      
    FTicksCor := aTempoCalc;
    FTicksPreco := aTempoCalc;
  end else
  if ((aTempoCalc-FTicksPreco) >= FCor.PPrecos^[FEtapa].prTempo) then begin
    { Não mudou a cor. Mas o tempo já é maior que o definido para essa etapa }
    if (FEtapa < High(FCor.PPrecos^)) then begin
      { Ainda existe existe mais etapas nessa cor. Devemos avançar para próxima então }
      Inc(FEtapa);
      FTicksPreco := aTempoCalc;
    end else
    { Não existe mais etapas nessa cor. Manter-se então nessa etapa ou Reiniciar da 
      1a caso essa cor esteja marcada para isso }
    if (Length(FCor.PPrecos^)>0) and FCor.Reinicia then
      FEtapa := 0;  
  end;
    
  if (FEtapa>=0) then 
  with FCor.PPrecos^[FEtapa] do begin
...
constructor TCMTarifador.Create;
begin
  FCreditoTotal := 0;
  FPassaportes := nil;
  FNumTicks := 0;
...
procedure TCMTarifador.SetCores(const Value: TCMListaCorPreco);
begin
  if FCores=Value then Exit;
  FCores := Value;
  FResetar := True;
end;

procedure TCMTarifador.SetCredito(const Value: Cardinal);
begin
...
procedure TCMTarifador.SetHoraCor(const Value: PCMHoraCor);
begin
  if FHoraCor=Value then Exit;
  FHoraCor := Value;
  FResetar := True;
end;

procedure TCMTarifador.SetInicio(const Value: TDateTime);
begin
...
procedure TCMTarifador.SetPassaportes(const Value: PPassaportes);
begin
...
procedure TCMTarifador.SetTipoAcesso(const Value: Word);
begin
...
procedure TCMTarifador.SetCredValor(const Value: Double);
begin
...
function TCMTarifador.TempoCredValor: Cardinal;
begin
...
function TCMTarifador.CalculaEtapaTempoValor(aTempo, aTol: Cardinal; aPreco: Double): Boolean;
var
  RestoI: Integer;
begin
  Result := False;
...
function TCMTarifador.CalculaEtapaTempoCred(aTempo, aTol: Cardinal;aPreco: Double): Boolean;
var
  RestoI: Integer;
  TempoP : Cardinal;
begin
...
function TCMTarifador.GetCores: TCMListaCorPreco;
begin
  Result := FCores;
end;

function TCMTarifador.GetCredito: Cardinal;
begin
  Result := FCredito;
end;

function TCMTarifador.GetCredValor: Double;
begin
  Result := FCredValor;
end;

function TCMTarifador.GetHoraCor: PCMHoraCor;
begin
  Result := FHoraCor;
end;

function TCMTarifador.GetInicio: TDateTime;
begin
  Result := FInicio;
end;

function TCMTarifador.GetNumTicks: Cardinal;
begin
  Result := FNumTicks;
end;

function TCMTarifador.GetPassaportes: PPassaportes;
begin
  Result := FPassaportes;
end;

function TCMTarifador.GetTipoAcesso: Word;
begin
  Result := FTipoAcesso;
end;

function TCMTarifador.GetTipoCalc: Byte;
begin
  Result := FTipoCalc;
end;

function TCMTarifador.GetValor: Double;
begin             
  if FIsento then
    Result := 0
  else  
...
function TCMTarifador.CreditoUsado: Cardinal;
begin
  Result := FCredito - FCreditoR;
end;

function TCMTarifador.GetIsento: Boolean;
begin
  Result := FIsento;
end;

procedure TCMTarifador.SetIsento(const Value: Boolean);
begin
  FIsento := Value;
end;

procedure TCMTarifador.CalculaCreditoTotal;
...
function TCMTarifador.Cronometro: Cardinal;
begin
...
function TCMTarifador.CronometroStr: String;
begin

end;

procedure TCMTarifador.Reset;
begin
  if FPassaportes<>nil then
    CalculaCreditoTotal
  else begin
...
procedure TCMTarifador.ZeraVars;
begin
  FTempoRes   := 0;
  FTol        := 0;
  FPrecoRes   := 0;
...
procedure TCMPausas.Adiciona(aInicio, aDuracao: Cardinal);
begin
...
constructor TCMPausas.Create;
begin
  SetLength(FPausas, 0);
end;

destructor TCMPausas.Destroy;
begin
  SetLength(FPausas, 0);
  inherited;
end;

function TCMPausas.DTTotal: TDateTime;
begin
  Result := TicksToDateTime(TicksTotal);
end;

function TCMPausas.GetItem(Index: Integer): PCMPausa;
begin
  Result := @FPausas[pred(Index)];
end;

function TCMPausas.GetQuantidade: Integer;
begin
  Result := Length(FPausas);
end;

function TCMPausas.GetString: String;
var I : Integer;
begin
  Result := '';
...
procedure TCMPausas.SetString(const Value: String);
var 
  S : String;
  P, Q : Integer;
  sInicio, sDur : String;
...
function TCMPausas.TicksTotal: Cardinal;
var I : Integer;
begin
  Result := 0;
