...
{$I dxSkins.inc}
  ...
  cxButtonEdit, cxHyperLinkEdit, cxMaskEdit, cxImage, cxDBLookupComboBox, cxMRUEdit, Db, DBTables,
  ImgList, dxmdaset, cxLookAndFeels, dxSkinsForm, dxSkinsCore, dxSkinsdxRibbonPainter,
  dxSkinsdxBarPainter, dxBarSkinnedCustForm, dxRibbonGallery,
...
type
  ...
  TcxSkinControllerStorage = record
    NativeStyle: Boolean;
  ...
  private
    FRibbon: TdxRibbon;
    FSkinControllerStorage: TcxSkinControllerStorage;
    FUpdateLock: Boolean;

    procedure CreateFrames;
    procedure RibbonHelpButtonClickHandler(Sender: TdxCustomRibbon);
    ...
    procedure RestoreSkinProperties;
    procedure StoreSkinProperties;
...
uses
  ...
  InPlaceEditorsDemoCheckBoxes, dxDemoUtils, dxAboutDemo;
...
begin
  ...
  try
    ...
    CreateSkinsMenuItems(BarManager, siStyles, dxSkinController1);
...
procedure TfrmMain.RibbonHelpButtonClickHandler(Sender: TdxCustomRibbon);
begin
  dxShowAboutForm;
end;
...
begin
  ...
  dxSkinController1.SkinName := FRibbon.ColorSchemeName;
end;

procedure TfrmMain.UpdateCheckBoxesView;
begin
  TfrmCheckBoxes(AFrameManager[Ord(bfcCheckBoxes)]).SetParameters(
    rgSelectColor.EditValue, chgSelectColor.EditValue, chbMonochrome.EditValue);
end;
...
procedure TfrmMain.RestoreSkinProperties;
begin
  with FSkinControllerStorage do
  begin
    dxSkinController1.NativeStyle := NativeStyle;
    dxSkinController1.UseSkins := UseSkins;
    dxSkinController1.Kind := Kind;
    dxSkinController1.SkinName := SkinName;
  end;
end;

procedure TfrmMain.StoreSkinProperties;
begin
  with FSkinControllerStorage do
  begin
    NativeStyle := dxSkinController1.NativeStyle;
    UseSkins := dxSkinController1.UseSkins;
    Kind := dxSkinController1.Kind;
    SkinName := dxSkinController1.SkinName;
  end;
end;
...
begin
  ...
  begin
    ...
    try
      ...
      tlbRibbonOptions.ItemLinks.Clear;
      tlbRibbonOptions.ItemLinks.Add(dxSkinChooserGalleryItem1);
      tlbRibbonOptions.ItemLinks.Add(btnRibbonStyle);
      ...
      dxSkinChooserGalleryItem1.SelectedSkinName := 'Blue';
      SelectTab(1);
    finally
      BarManager.EndUpdate;
    end;
...
begin
  ...
  FRibbon.OnHelpButtonClick := RibbonHelpButtonClickHandler;
...
begin
  tblContacts.FieldByName('FullName').Value :=
    tblContacts.FieldByName('FirstName').AsString + ' ' +
    tblContacts.FieldByName('LastName').AsString;
end;
