...
type
  ...
  public
    ...
    procedure SetaDB(DB: TnxDatabase);
...
begin
  ...
  while not qCaixa.Eof do begin
    ...
    try
      ...
      if tCaixa.FindKey([qCaixaNumero.Value]) then
        tCaixa.Edit
      else begin
        tCaixa.Insert;
        tCaixaNumero.Value := qCaixaNumero.Value;
        tCaixaAberto.Value := False;

        tAcesso.IndexName := 'ICaixaFim';
        if tAcesso.FindKey([qCaixaNumero.Value]) then begin
          tCaixaAbertura.Value := tAcessoInicio.Value;
          tCaixaFechamento.Value := tAcessoInicio.Value;
        end;
...
procedure TdmConv15.SetaDB(DB: TnxDatabase);
begin
  tAcesso.Database := DB;
  tTran.Database := DB;
  tCaixa.Database := DB;
