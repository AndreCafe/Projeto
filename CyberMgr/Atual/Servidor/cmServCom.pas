...
procedure TCMServComunicacao.nmEnviaChat(var Msg: TCSCMessage);
var 
  S: TStream;
  SL: TStrings;
  De, Para: Integer;
    ...
    try
      try
        case nmTipoClasse of
          ...
          tcGrupo   : FCliente.Grupos.SalvaStream(S);
...
procedure TCMServComunicacao.nmObtemPastaServ(var Msg: TCSCMessage);
var 
  S : String;
  Erro : Integer;
  Reply : TnmNomeArq;
...
procedure TCMServComunicacao.nmArqFundoEnviado(var Msg: TCSCMessage);
begin
...
procedure TCMServComunicacao.tcChatEv(var Msg: TMessage);
var S: TStream;
begin
  S := TStream(Msg.lParam);
  if S <> nil then begin
    S.Position := 0;
    EnviaEvento(cmnmChatEv, S, S.Size, nmdStream);
  end;  
end;
...
procedure TCMServComunicacao.nmLimpaFundo(var Msg: TCSCMessage);
begin
...
procedure TCMServComunicacao.nmObtemSenhaCli(var Msg: TCSCMessage);
var 
  Erro: Integer;
  Reply: TnmSenhaCli;
  S: String;
...
procedure TCMServComunicacao.nmSalvaSenhaCli(var Msg: TCSCMessage);
var Erro: Integer;
begin
  with Msg, TnmSenhaCli(dmData^) do begin
...
begin
  ...
  TCMServidor(Servidor).ChecaNumMaq;
end;

procedure TCMServComunicacao.nmAdicionaPassaporte(var Msg: TCSCMessage);
begin
...
procedure TCMServComunicacao.nmPaginasImpressas(var Msg: TCSCMessage);
begin
...
procedure TCMServComunicacao.nmAvisos(var Msg: TCSCMessage);
var 
  S: TStream;
  Erro: Integer;
begin
