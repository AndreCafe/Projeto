...
begin
  if Trim(edNome.Text)='' then begin
    edNome.SetFocus;
    Raise Exception.Create('É necessário informar o nome da loja');
  end;

  if Trim(edProp.Text)='' then begin
    edProp.SetFocus;
    Raise Exception.Create('É necessário informar o nome do proprietário da loja');
  end;

  if Trim(edSenha.Text)='' then begin
    edSenha.SetFocus;
    Raise Exception.Create('É necessário informar uma senha de registro');
  end;

  if Trim(edEmail.Text)='' then begin
    edEmail.SetFocus;
    Raise Exception.Create('É necessário informar o seu E-mail');
  end;

  if edEmail.Text <> edREmail.Text then begin
    edREmail.SetFocus;
    Raise Exception.Create('Você digitou 2 e-mails diferentes. É necessário repetir o seu e-mail corretamente');
  end;

  if edSenha.Text <> edRSenha.Text then begin
    edSenha.SetFocus;
    Raise Exception.Create('As senhas informadas estão diferentes. Certifique-se de digitar de forma idêntica a senha e a repetição da senha');
  end;
