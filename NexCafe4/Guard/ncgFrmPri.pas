...
uses
  ...
  LMDCustomPanelFill, LMDPanelFill, LMDPNGImage, ShlObj, cxShellCommon, ncgAppbar,
...
uses 
  ...
  ncgFrmPanFree, cadVersionInfo, ncgFrmYAD, ncAppUrlLog;
...
begin
  ...
  if P.ItemCount>0 then
    P.Terminate;  
  P := Processes('explorer.exe');
  if P.ItemCount>0 then FechaProc(P[0].ID);
end;
...
begin
  try
    try
      if (LastKillExp>0) and (GetTickCount-LastKillExp>10000) then
...
//        visValor(False);
    ...
    if (FrmAlteraSenha=nil) and gConfig.PMPausaAutomatica and gConfig.PMConfirmaImpCliente and (S<>nil) and CM.Ativo and (Status=sgSessao) then begin
      J := CM.Jobs.JobPendMaq(CM.Maquina, True);
      FrmImp.Ativo := (J<>nil) and (J.ClientPages>0);
    end else
      FrmImp.Ativo := False;

    if Assigned(FrmAlteraSenha) or FrmImp.Ativo then 
...
begin
  ...
  if Assigned(FrmYAD) and doShowAD then 
    FrmYAD.RecreateAppBar;
end;
...
begin
  Exit;
{  
  if (not FPrintHookOk) and (not ShieldInjOk) then Exit;
  ...
  if slUnhook.IndexOf(IntToStr(aProcessID))=-1 then begin
    ...
    try
      ...
      if ShieldInjOk then
        UninjectLibrary(ph, 'guardshield.dll');
    finally
      CloseHandle(ph);
    end;
  end;}
...
procedure _Fechar(P: IProcess);
begin
  if (FN='?') or SameText(FN, 'NEXGUARD.EXE') or SameText(FN, 'EXPLORER.EXE') then Exit;
  UnhookProc(P.ID);
  case aEtapa of
    0 : P.Close;
  ...
  try
    ...
    for I := 0 to pred(P.ItemCount) do 
      if SessionID=P[I].Session then begin
        ...
        DebugMsg('TFrmPri.FechaProgramas - 10');
...
begin
  ...
  if FShellNex=ShellDef then
    ...
    ChangeShellData(gConfig.FiltrarDesktop, gConfig.FiltrarMenuIniciar);
...
begin
  if (((M<>nil) and (M.Direito=dmFree)) or gConfig.PubHomePage) then begin
    FHomeOpened := True;
    ShellStartCustom('http://open.nextar.com.br', '', '', 0, SW_SHOWMAXIMIZED);
  end;
end;
...
begin
  DebugMsg('TFrmPri.TimerShieldHookTimer');
  try
      ...
      try
        ...
        try
          ...
          if S<>'' then begin
            ...
            AlterouSite := True;
            if (Site<>'') and SameHost(Site, FUFechaHost) then
              FechaBrowser(brIE,0);
          end;  
        finally
          DDE.CloseLink;
