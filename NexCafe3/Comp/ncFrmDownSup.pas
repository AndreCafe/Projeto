...
uses
  ...
  ComCtrls, ncClassesBase, ExtCtrls;
...
begin
  Result := ExtractFilePath(ParamStr(0))+'\suporte';
end;
...
begin
  ...
  DownURL.URL := 'https://secure.logmeinrescue.com/Customer/Download.aspx?EntryID=1856965867'+
                 '&comment0='+aNome+
                 '&comment1='+gConfig.CodLoja+
                 '&comment2='+aobs;
  LimpaDirSuporte;                 
  ShowModal;                 
end;
...
begin
  ...
  if not DirectoryExists(DirSup) then
    MkDir(DirSup);
  DownURL.Filename := DirSup + '\SupRem' + IntToStr(Random(MaxInt)) + '.exe';
