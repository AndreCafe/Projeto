...
type
  TFrmCopia = class(TForm)
    ...
    btnReindex: TPngSpeedButton;
    edPastaCM: TLMDBrowseEdit;
    cxLabel5: TcxLabel;
...
begin
  ...
  if wnd=0 then
    Raise ENexCafe.Create('É necessário estar com o servidor do Cyber Manager (CM-Server) em executação nessa máquina para realizar essa operação.');
...
begin
  if MessageDlg('Deseja executar o processo de recuperação de banco de dados do NexCafé ?', mtConfirmation, [mbYes, mbNo], 0, mbNo)<>mrYes then Exit;
...
begin
  ...
  try
    ...
    try
      ...
      try
        try
          dmCM.OpenConn(edPastaCM.Text);
        except
          raise ENexCafe.Create('Não foi possivel importar os dados do Cyber Manager. '+
                                 'Certifique-se que a pasta informada esteja correta e contenha os dados.');
        end;
