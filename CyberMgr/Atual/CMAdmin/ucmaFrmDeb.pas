...
uses
  ...
  OvcBase, OvcRLbl, OvcPLb,
  dxBar, StdCtrls, OvcEditF, OvcEdPop, OvcEdCal,
  OvcDbDat, DBCtrls, OvcPB, OvcPF, OvcDbPF, OvcEF, OvcSF, OvcDbSF, ExtCtrls,
  ComCtrls, OvcDbDLb, lmdcctrl, lmdctrl,
  lmdstdcS, Lmddbctr, Db, kbmMemTable, 
  lmdgroup, lmddbctA, nxdb, 
  Mask, 
  ...
  dxBarExtItems,
  LMDCustomSimpleLabel, LMDSimpleLabel, cxStyles, cxCustomData,
  cxGraphics, cxFilter, cxData, cxDataStorage, cxEdit, cxDBData,
  cxCheckBox, cxImageComboBox, cxMaskEdit, cxCalendar, cxTimeEdit,
  cxCurrencyEdit, cxGridCustomTableView, cxGridTableView,
  cxGridDBTableView, cxClasses, cxControls, cxGridCustomView, cxGridLevel,
  cxGrid, cxDBLookupComboBox, cxContainer, cxTextEdit, cxMemo, cxDBEdit,
  cxVGrid, cxDBVGrid, cxInplaceContainer, cxDBExtLookupComboBox;
...
type
  TFrmDeb = class(TForm)
    ...
    InspCabDBEditorRow1: TcxDBEditorRow;
    InspCabData: TcxDBEditorRow;
    InspCabNomeUsuario: TcxDBEditorRow;
    edObs: TcxDBMemo;
    vgTot: TcxDBVerticalGrid;
    vgTotDebTot: TcxDBEditorRow;
    vgTotDebSel: TcxDBEditorRow;
    vgTotDesconto: TcxDBEditorRow;
    vgTotDBEditorRow4: TcxDBEditorRow;
...
uses cmClassesBase, ufmImagens, ucmaDM, ucmaPri;
...
begin
  PostInspectors(Self);
  if (mtTranDesconto.Value - mtTranDebSel.Value) > 0.00009 then begin
    Beep;
  ...
  try
    ...
    if Abs(mtTranDesconto.Value-mtTranDebSel.Value) < 0.001 then begin
      tTranDesconto.Value := FSelAcesso;
      tTranDescontoProduto.Value := FSelVendas;
    end else begin  
      tTranDesconto.Value := DuasCasas((mtTranDesconto.Value / mtTranDebSel.Value) * FSelAcesso);
...
procedure TFrmDeb.TVCustomDrawCell(Sender: TcxCustomGridTableView;
  ACanvas: TcxCanvas; AViewInfo: TcxGridTableDataCellViewInfo;
  var ADone: Boolean);
var V : Variant;  
begin
  V := AViewInfo.GridRecord.Values[TVPagar.Index];
  if (V<>Null) and (V=True) then
    ACanvas.Font.Style := [fsBold];
end;

procedure TFrmDeb.vgTotDescontoEditPropertiesChange(Sender: TObject);
begin
