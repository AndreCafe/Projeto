...
uses
  ...
  Variants,
  {$endif}
  dxDBTLCl, dxGrClms, ExtCtrls, ImgList, Db, ffdb, dxExEdtr;
...
uses cmIDRecursos, uCMFrmCredito, uCMDados, cmClassesBase, uCMFrmCapt, uCMFrmAcesso,
  uCMFrmPri, uCMAShutdown, uCMFrmVendaPac;
...
begin
  ...
  if M.Acesso>0 then begin
    ...
    if M.AguardaPagto then begin
      ...
      if (M.CreditoCli>5000) or (M.Sinal>0.0009) then begin
...
begin
  if Dados.CM.Config.PermiteCapturaTela then
...
begin
  inherited;
  cmNovo.Enabled := Permitido(reMaquinas, taMaqCadastrar);
  cmApagar.Enabled := Permitido(reMaquinas, taMaqApagar); 
end;
