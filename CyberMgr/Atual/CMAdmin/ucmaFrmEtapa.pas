...
uses
  ...
  LMDCustomParentPanel, LMDCustomPanelFill, LMDButtonPanel, ComCtrls, ExTrackBar,
  ...
  kbmMemTable, StdCtrls, cxMaskEdit;
...
uses ucmaDM, ucmaPri, ufmImagens;
...
begin
  ...
  try
    ...
    try
      ...
      begin  
        TF := edValor.AsFloat - FEtapaA^.erValor;  
        if TF > 999999 then  
          Raise Exception.Create('Valor não pode ser maior que 999999');  
        I := 0;  
        F := TF;
        while (F>0.01) do begin
          Inc(I);
          if I=1 then begin
            mtFrac.Append;
            mtFracValorFrac.Value := TF;
            mtFracTexto.Value := 'Não dividir em sub-etapas';
            mtFrac.Post;
          end else begin
            F := TF / I;
            if Frac(F*100) < 0.000000001 then begin
              mtFrac.Append;
              mtFracValorFrac.Value := F;
              mtFracTexto.Value := 'Cobrar ' + FloatToStrF(F, ffCurrency, 10, 2) + ' a cada ' + TempoSt(FormatDateTime('hh:mm:ss', T / I));
              mtFrac.Post;
            end;
          end;
        end;
...
begin
  ...
  try
    ...
    while (ValorF-Valor) > 0.001 do begin
      mtSim.Append;
