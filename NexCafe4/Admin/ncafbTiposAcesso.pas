...
begin
  ...
  if Tab.RecordCount=1 then begin
    ShowMessage('Não é possível apagar todos grupos de tarifas');
    Exit;
  end;
  T := gTiposAcesso.PorCodigo[TabID.Value];
  if T<>nil then
  if SimNaoH('Deseja realmente apagar o grupo ' + TabNome.Value + '?', Handle) then
  begin
    Dados.CM.ApagaObj(T);
    Tab.Refresh;
  end;
