object FrmTelefone: TFrmTelefone
  Left = 243
  Top = 334
  BorderStyle = bsDialog
  Caption = 'Telefone'
  ClientHeight = 179
  ClientWidth = 361
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  KeyPreview = True
  OldCreateOrder = False
  Position = poScreenCenter
  OnClose = FormClose
  OnKeyDown = FormKeyDown
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object LMDSimplePanel1: TLMDSimplePanel
    Left = 0
    Top = 22
    Width = 361
    Height = 157
    UseDockManager = False
    Align = alClient
    Bevel.BorderInnerWidth = 5
    Bevel.Mode = bmStandard
    Bevel.StandardStyle = lsNone
    TabOrder = 4
    object InspTel: TdxDBInspector
      Left = 5
      Top = 5
      Width = 351
      Height = 147
      Align = alClient
      DataSource = FrmCliente.dsTel
      DefaultFields = False
      TabOrder = 0
      DividerPos = 69
      Flat = True
      Options = [dioAutoWidth, dioBandSizing, dioCancelOnExit, dioColumnSizing, dioDynamicColumnSizing, dioEditing, dioEnterThrough]
      PaintStyle = ipsExtended
      Data = {
        
        000000}
      object InspTelTelefone: TdxInspectorDBMaskRow
        FieldName = 'Telefone'
      end
      object InspTelRamal: TdxInspectorDBMaskRow
        FieldName = 'Ramal'
      end
      object InspTelUF: TdxInspectorDBPickRow
        Caption = 'UF'
        CharCase = ecUpperCase
        DropDownListStyle = True
        Items.Strings = (
          'AC'
          'AL'
          'AM'
          'AP'
          'BA'
          'CE'
          'DF'
          'ES'
          'GO'
          'MA'
          'MG'
          'MS'
          'MT'
          'PA'
          'PB'
          'PE'
          'PI'
          'PR'
          'RJ'
          'RN'
          'RO'
          'RR'
          'RS'
          'SC'
          'SE'
          'SP'
          'TO')
        CanDeleteText = True
        FieldName = 'Uf'
      end
      object InspTelcidade: TdxInspectorDBExtLookupRow
        CharCase = ecUpperCase
        DBGridLayout = FrmCliente.layoutCidade
        PickListField = 'Cidade'
        FieldName = 'cidade'
      end
      object InspTelObs: TdxInspectorDBMemoRow
        RowHeight = 50
        FieldName = 'Obs'
      end
    end
  end
  object BarMgr: TdxBarManager
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    Bars = <
      item
        AllowClose = False
        AllowCustomizing = False
        AllowQuickCustomizing = False
        AllowReset = False
        BorderStyle = bbsNone
        Caption = 'Controles'
        DockedDockingStyle = dsTop
        DockedLeft = 0
        DockedTop = 0
        DockingStyle = dsTop
        FloatLeft = 67
        FloatTop = 344
        FloatClientWidth = 69
        FloatClientHeight = 44
        ItemLinks = <
          item
            Item = cmGravar
            Visible = True
          end
          item
            Item = cmCancelar
            Visible = True
          end>
        Name = 'Controles'
        NotDocking = [dsNone, dsLeft, dsTop, dsRight, dsBottom]
        OneOnRow = True
        Row = 0
        UseOwnFont = False
        Visible = True
        WholeRow = True
      end>
    Categories.Strings = (
      'Default')
    Categories.ItemsVisibles = (
      2)
    Categories.Visibles = (
      True)
    PopupMenuLinks = <>
    UseSystemFont = True
    Left = 112
    Top = 120
    DockControlHeights = (
      0
      0
      22
      0)
    object cmGravar: TdxBarButton
      Caption = '&Gravar'
      Category = 0
      Hint = 'Gravar'
      Visible = ivAlways
      Glyph.Data = {
        
        3F3F373F3F00}
      PaintStyle = psCaptionGlyph
      OnClick = cmGravarClick
    end
    object cmCancelar: TdxBarButton
      Caption = '&Cancelar'
      Category = 0
      Hint = 'Cancelar'
      Visible = ivAlways
      Glyph.Data = {
        
        777770000000}
      PaintStyle = psCaptionGlyph
      OnClick = cmCancelarClick
    end
  end
end
