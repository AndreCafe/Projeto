unit uTMFrmDadosGraf;

interface

uses
  Windows, Messages, SysUtils, Classes, Graphics, Controls, Forms, Dialogs,
  StdCtrls, Buttons, dxCntner, dxTL, dxDBCtrl, dxDBGrid, OvcBase, lmdstdcA,
...
begin
  with INI do begin
    WriteInteger('Totalização', 'NumCampos', tiNumCampos);
    
    for I := 1 to 3 do
    WriteString('', 'Campo'+IntToStr(I), tiCampo[I]);

    WriteString('', 'Chave', tiChave);
    WriteInteger('', 'MaxCols', tiMaxCols);
    WriteBool('', 'MostraOutros', tiMostraOutros);
    WriteBool('', 'MostraMedia', tiMostraMedia);
    WriteInteger('', 'Ordem', tiOrdem);
    WriteString('', 'Titulo', tiTitulo);

    for I := 1 to 25 do 
    WriteString('', 'Coluna'+IntToStr(I), tiColunas[I]);
  end;  
end;
...
begin
  with INI do begin
    ...
    for I := 1 to 3 do
    tiCampo[I] := ReadString('', 'Campo'+IntToStr(I), tiCampo[I]);

    tiChave := ReadString('', 'Chave', tiChave);
    tiMaxCols := ReadInteger('', 'MaxCols', tiMaxCols);
    tiMostraOutros := ReadBool('', 'MostraOutros', tiMostraOutros);
    tiMostraMedia := ReadBool('', 'MostraMedia', tiMostraMedia);
    tiOrdem := ReadInteger('', 'Ordem', tiOrdem);
    tiTitulo := ReadString('', 'Titulo', '');

    for I := 1 to 25 do 
    tiColunas[I] := ReadString('', 'Coluna'+IntToStr(I), '');
  end;  
end;
...
begin
  ...
  with tiAtual do begin
    ...
    cbCampo3.ItemIndex := cbCampo3.Items.IndexOf(ColFieldNameToCaption(grid, tiCampo[3]));
...
begin
  ...
  cbCampo3.Enabled := (cbCampo1.ItemIndex>0) and (cbCampo2.ItemIndex>0);
  cbCampo2.Enabled := (cbCampo1.ItemIndex>0);
  if cbCampo3.Enabled and (cbCampo3.ItemIndex>0) then
    tiAtual.tiNumCampos := 3
  else
