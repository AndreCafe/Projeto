    ...
    if FIniciar then begin
      ...
      if FSessao.Encerrou  then begin
        ...
        if (vgTDesc.Properties.Value - FSessao.TotalGeral)  > 0.001 then
          Raise ENexCafe.Create('Desconto não pode ser maior que o total');
        ...
        if (Debitar>0.009) then begin 
          ...
          if (FSessao.Cliente=0) then
            Raise ENexCafe.Create('É necessário que tenha um cadastro para poder deixar débito');
          if Debitar > 0.009 then begin
...
begin
  ...
  if aSessao=nil then
    cmMaq.Caption := '' else
    cmMaq.Caption := 'Máq. '+IntToStr(aSessao.Maq);
  FSessao := aSessao;
