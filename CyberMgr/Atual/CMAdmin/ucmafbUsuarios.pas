...
uses ucmaDM, ucmaPri, ucmaFrmUsuario, cmClassesBase;
...
begin
  with Dados do begin
    ...
    if mtUsuarioAdmin.Value and (C<2) then
      Raise Exception.Create('Não é possível apagar o único usuário administrador do sistema');
        
    if SimNaoH('Confirma a exclusão de '+mtUsuarioNome.Value+'?', Handle) then begin
