...
uses
  FrameIDs, dxFrames, dxGridFrame, maindata, uStrsConst;
...
begin
  ...
  if AShow then
  begin
    Grid1.Width := plGrid.Width div 2;
...
begin
  pt := plGrid.ScreenToClient(pt);
  if PtInRect(Grid.BoundsRect, pt) then
    Result :=  GridDBTableView
  else
    if PtInRect(Grid1.BoundsRect, pt) then
