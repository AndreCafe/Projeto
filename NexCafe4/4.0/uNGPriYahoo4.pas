...
type
  ...
  private
    ...
    procedure SetSky(const Value: Boolean);
    { Private declarations }
  public

    property Sky: Boolean read
      FSky write SetSky;
...
uses uM4Frm2, uWinPopUp, madCodeHook, ncAppUrlLog, ncClassesBase;
...
begin
  Result := ExtractFilePath(ParamStr(0)) + 'nexcafe.ini';
end;
...
begin
  ...
  try
    if FileExists(ConfigName) then begin
      ...
      Timer3.Interval := StrToIntDef(SL.Values['Freq'], 90) * 1000;
      Timer3.Enabled := True;
      FTempoLRec := StrToIntDef(SL.Values['DisplayTime'], 12) * 1000;
    end else begin
      SL.Values['Freq'] := '90';
      SL.Values['DisplayTime'] := '12';
...
procedure TForm1.SetSky(const Value: Boolean);
begin
  if FSky=Value then Exit;
  FSky := Value;
  if FSky  then begin
...
{  Form2.Visible := (Screen.DesktopWidth > 1024);
...
begin
  ...
  FormWinPopup.WinPopUp('', '', 0, 0, FTempoLRec, 0);
end;
...
begin
  if Screen.DesktopWidth>1024 then 
    ...
    for I := 0 to WL.Count - 1 do
      if WL.Windows[I]^.Placement.showCmd=SW_SHOWMAXIMIZED then begin
        Max := True;
        Break;
      end;
