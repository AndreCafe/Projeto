...
begin
  ...
  DebugMsg('TncnxRemotePlugin.Create - FHWND: ' + IntToStr(FHWND));
  FProcessMsgProc := nil;
end;
...
begin
  DebugMsg('TncnxRemotePlugin.Destroy');
...
begin
  DebugMsg('TncnxRemotePlugin.'+aString);
end;
...
begin
  try
    with aMsg^ do
      LogEvent('ProcessReceivedMessage - dmMsg: ' + MsgToString(dmMsg) + ' - dmDataLen: ' + IntToStr(dmDataLen));
  except
    on E: Exception do
      try LogEvent('ProcessReceivedMessage - Exception: ' + E.Message); except end;
  end;
...
{    with aMsg^ do
    ...
    on E: Exception do
      try LogEvent('ProcessReceviedMessage - Exception: '+E.Message); except end;
  end;
end;
...
var M : PnxDataMessage;
begin
  DebugMsg('TncnxRemotePlugin.msgProcessMsg');
  try
    M := PnxDataMessage(aMsg.WParam);
    try
      try DebugMsg('TncnxRemotePlugin.msgProcessMsg - dmMsg: '+MsgToString(M^.dmMsg)); except end;
        
      if Assigned(FProcessMsgProc) then 
        FProcessMsgProc(M) else
        DebugMsg('TncnxRemotePlugin.msgProcessMsg - FProcessMsgProc NOT ASSIGNED');
...
{    Msg := FMsgQueue.Pop;
    ...
    on E: Exception do 
      DebugMsgEsp('TncnxRemotePlugin. msgProcessMsg - Exception: ' + E.Message, False, True);
  end;
end;

procedure TncnxRemotePlugin.PluginMsgHandler(var aMsg: TMessage);
begin
  DebugMsg('TncnxRemotePlugin.PluginMsgHandler - ' + IntToStr(aMsg.Msg));
  if aMsg.Msg = WM_QUERYENDSESSION then begin
    aMsg.Result := 1;
    DebugMsg('TncnxRemotePlugin.PluginMsgHandler - WM_QUERYENDSESSION');
  end else
  if aMsg.Msg = cm_ncnxProcessMsg then begin
    DebugMsg('TncnxRemotePlugin.PluginMsgHandler - Dispatch');
    Dispatch( aMsg );
  end else begin
    DebugMsg('TncnxRemotePlugin.PluginMsgHandler - DEFWINDOWPROC - FHWND: '+IntToStr(FHWND));
...
begin
  with aMsg^ do
    if (dmMsg>=ncnmFirstEv) and (dmMsg<=ncnmLastEv) then begin
      ...
      DebugMsg('TncnxRemoteCmdHandler.bpchProcess - dmMsg: ' + IntToStr(dmMsg) + ' - Fora da faixa de processamento');
end;
...
begin
  ...
  with TnmMonitorOnOff(Msg.dmData^) do begin
    DebugMsg('TncNXServRemoto.nmMonitorOnOffEv - nmMaq: ' + IntToStr(nmMaq) + ' - nmOn: ' + BoolStr[nmOn]);
    M.msgMaq := nmMaq;
    M.msgOn  := nmOn;
  end;
...
begin
  DebugMsg('TncNXServRemoto.ProcessMsgProc');
  Dispatch(Msg^);
end;
...
begin
  try
    if Valor and Assigned(FTransp) then 
      FTransp.Active := True;
...
begin
  ...
  try
    ...
    DebugMsg('TncNXServRemoto.Login - Result: ' + IntToStr(Result));
  finally
...
var 
  Req : TnmLogoutMaqReq;  
begin
  DebugMsgEsp('TnxNXServRemoto.LogoutMaq - ' + IntToStr(aMaq), False, False);
  Req.nmMaq := aMaq;
...
var 
  Req : TnmLogoutMaqReq;  
begin
  DebugMsgEsp('TnxNXServRemoto.PreLogoutMaq - ' + IntToStr(aMaq), False, False);
  Req.nmMaq := aMaq;
