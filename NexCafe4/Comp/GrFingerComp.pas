...
begin
  if Assigned(GlobalGrComp) then
    raise Exception.Create('Já existe um TGrFingerComp instanciado');
...
begin
  if (not SameText(idSensor, 'File')) and (event=gr_plug) then begin
    FSensor := idSensor;
...
begin
  ...
  if Value then begin
    LoadGrFingerLib;
    GrFinalize;
    if GrInitialize<>GR_OK then Exit;
    if GrCapInitialize(CapInitCallback)<>GR_OK then Exit;
  end else begin
    GrCapFinalize;
    GrFinalize;
  end;
...
procedure Register;
begin
  RegisterComponents('NexCafe', [TGrFingerComp]);
end;
