...
begin
  if gbTexto.Enabled and (Trim(edTexto.Text)='') then
    raise ENexCafe.Create('É necessário digitar o texto que será exibido ao cliente');

  if gbSom.Enabled and (TabSom.IsNull) then 
    raise ENexCafe.Create('É necessário selecionar um arquivo de som a ser tocado no momento do aviso');

  if (edMinutos.Value=null) or (edMinutos.Value=0) then
    raise ENexCafe.Create('É necessário informar a quantidade de minutos');
...
begin
  if TabSom.BlobSize = 0 then 
    Raise ENexCafe.Create('O arquivo de som ainda não foi selecionado');
