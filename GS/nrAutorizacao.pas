...
type  
  ...
  TCabAutorizacao = class
    ...
    procedure Save(UsuarioAtual: String; B: TCabAutorizacao; tCli, tAut: TnxTable; aStartTran: Boolean);
  end;
...
begin
  ...
  if (tCli.FieldByName('Codigo').AsInteger<>aCodloja) and (not tCli.FindKey([aCodLoja])) then
    Raise Exception.Create('Loja não encontrada ('+IntToStr(aCodLoja)+')');
...
procedure TCabAutorizacao.Save(UsuarioAtual: String; B: TCabAutorizacao; tCli, tAut: TnxTable; aStartTran: Boolean);
var 
  I, T : Integer;
  IB : TAutorizacao;
  ACab, Inativar, AlterouItem, NeedTran : Boolean;
  L : TLicenca;
begin
  ...
  T := 0;
  for I := 0 to Count-1 do with Item(I) do
    if auTipo=ectcFreePremium then begin
      T := 1;
      caMaquinas := 1;
      Break
    end else
  ...
  if aStartTran then tCli.Database.StartTransactionWith([tCli, tAut]);
  try
    tAut.CancelRange;
    tAut.IndexName := 'IID';
    ACab := AlterouCab(B);
    ...
    if aStartTran then tCli.Database.Commit;
  except
    if aStartTran then tCli.Database.Rollback;
