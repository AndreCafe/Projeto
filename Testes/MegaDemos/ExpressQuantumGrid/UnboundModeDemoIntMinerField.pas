...
type
  ...
  public
    ...
    property DescriptionHeight: Integer read FDescriptionHeight write FDescriptionHeight;
  end;
...
uses Extctrls, cxGridDBTableView, cxGridLevel
  {$IFDEF DELPHI6}
  ,Variants , Types
  {$ENDIF}
  , cxControls;
...
begin
  ...
  vType := VarType(AViewInfo.Value);
  if (vType = varEmpty) or (vType = varNull) then Exit;
  CellRec := PCellStateRec(Integer(AViewInfo.Value))^;
...
begin
  with (Views[0] as TcxGridTableView) do
    Result := PCellStateRec(Integer(ViewInfo.RecordsViewInfo[ARow].GetCellViewInfoByItem(Items[ACol]).Value))^;
end;
