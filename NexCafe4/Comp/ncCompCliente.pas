  ...
  if Valor then begin
    ...
    try
      if FServidor=nil then 
        Raise ENexCafe.Create('TClienteNexCafe: É preciso informar o servidor do NexCafé');
...
begin
  ...
  DebugMsg('ClientCMWndFunc - HWND: ' + IntToStr(hWindow) + ' - Msg: ' + IntToStr(Msg) + ' - wParam: ' + IntToStr(wParam) + ' - lParam: ' + IntToStr(lParam));

  if MSG<>wm_Create then begin
    Obj := TClienteNexCafe(GetWindowLong(hWindow, 0));
    if Obj=nil then begin 
      Result := DefWndFunc;
      DebugMsg('ClienteCMWndFunc - Obj = nil');
      Exit;
    end;
  end;
  ...
  case Msg of
    ...
    begin
      ...
      M.WParam := WParam;
      M.LParam := LParam;
      DebugMsg('ClienteCMWndFunc - Obj.DespachaMC');
      Obj.DespachaMC(M);
    end;
    
  else 
    DebugMsg('ClienteCMWndFunc - DefWndFunc');
    Result := DefWndFunc;
  end;
end;
...
begin
  DebugMsg('TClienteNexCafe.tcAbriuFechouCaixaEv');
...
begin
  ...
  DebugMsg('TClienteNexCafe.tcAtualizaObj - 1');
  ...
  DebugMsg('TClienteNexCafe.tcAtualizaObj - TipoClasse: '+IntToStr(TipoClasse)+' - Chave: ' + Chave);

  if (TipoClasse in [tcTipoAcesso, tcTarifa]) and LadoServidor then Exit;
  
  if TipoClasse=tcConfig then begin
    DebugMsg('TClienteNexCafe.tcAtualizaObj.TipoClasse=tcConfig');
    ...
    DebugMsg('TClienteNexCafe.tcAtualizaObj - 3');
...
var 
  ...
  De, Para, ID: Integer;
begin
  DebugMsg('TClienteNexCafe.tcChatEv - 1');

  if not Assigned(FAoReceberChat) then Exit;

  DebugMsg('TClienteNexCafe.tcChatEv - 2');
  ...
  try
    ...
    SL.Delete(0);
    SL.Delete(0);
    DebugMsg('TClienteNexCafe.tcChatEv - Para: ' + IntToStr(Para));
...
begin
  with PmsgFinalizaProcesso(Msg.lParam)^ do begin  
    DebugMsg('TClienteNexCafe.tcFinalizaProcessoEv - ' + IntToStr(aMaq));
    aMaq := msgMaq;
...
begin
  DebugMsg('TClienteNexCafe.tcMonitorOnOffEv');
...
begin
  DebugMsg('TClienteNexCafe.tcNovoObj - 1');
  tcAtualizaObj(Msg);
end;
...
begin
  with PmsgObtemProcessos(Msg.lParam)^ do begin 
    DebugMsg('TClienteNexCafe.tcObtemProcessosEv - msgMaq: '+IntToStr(FMaquina));
...
begin
  with PmsgCapturaTela(Msg.LParam)^ do begin
    DebugMsg('TClienteNexCafe.tcPedeTela - msgMaq: '+IntToStr(FMaquina)+' - msgSession: ' + IntToStr(msgSession));
...
begin
  with PmsgShutdown(Msg.lParam)^ do begin
    DebugMsg('TClienteNexCafe.tcShutdow');
    M := MaquinaObj;
    if ((msgMaq=0) or 
        (msgMaq=FMaquina) or
...
begin
  ...
  DebugMsg('TClienteNexCafe.tcSiteBloqueadoEv');
...
begin
  try
    DebugMsg('TClienteNexCafe.DespachMC - Msg: ' + GetMsgIDString(Msg.Msg));
    Dispatch(Msg);
...
begin
  DebugMsgEsp('TClienteNexCafe.PreLogoutMaq - ' + IntToStr(aMaq), False, False);
...
begin
  DebugMsgEsp('TClienteNexCafe.LogoutMaq - ' + IntToStr(aMaq), False, False);
