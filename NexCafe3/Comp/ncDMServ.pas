...
type
  ...
  public
    ...
    procedure SalvaPaginasImpressas(aMaq: Word; Maq: TncMaquina; Sessao: TncSessao; aPaginas: Integer; aImp, aDoc: String; aRegistrar: Integer);
    ...
    function SalvaImpressao(Imp: TncImpressao; aSessoes: TncSessoes = nil): Integer;
    ...
    procedure PermitirDownload(S: TncSessao; aPermitir: Boolean);
...
uses ncEstruturaTabs, ncVersoes, ncDebug;
    ...
    if IM._Operacao=osAlterar then begin
      if Achou then begin
        ...
        if (AjustaSaldoPost(IM.imProduto, IM.imDataHora, IM.imID, IM.SaldoPost)<0) and
           (MEAtu.Tipo=trEstVenda) and gConfig.NaoVenderAlemEstoque then 
        begin
...
begin
  ...
  case S.Impressoes[k]._Operacao of
    ...
    osAlterar  : begin
      ...
      Result := SalvaImpressao(S.Impressoes[k]);
      if Result<>0 then Exit;
    end;
...
begin
  Result := 0;
  InitTran([tTran, tTempo, tCli, tRec, tITran, tSessao, tOcupacao, tPassaporte, tHPass, tDebito, tProduto, tMovEst, tCaixa], True);
  try
    Debitar := 0;
    ...
    try
      ...
      if (S.Cliente>0) and ((not S.Encerrou) or (S.TranF=0)) and (tSessaoCLiente.Value<>S.Cliente) and 
         (aSessoes.PorCliente[S.Cliente]<>nil) then 
      begin
...
{      if S.Cliente>0 then begin
        Debitar := Debitar + S.TotalGeral - S.DescTran - S.PagoTran;
        if (Debitar > 0.009) then begin
          tCli.Locate('ID', S.Cliente, []);
          if tCliDebito.Value > LimiteDeb then begin
...
begin
  Result := 0;
  NewTran := InitTran([tCaixa, tCli, tDebito, tTran, tITran, tImp, tMovEst, tTempo, tProduto, tHPass, tPassaporte, tSessao], True);
  try
    try
      NumCx := NumCaixaAberto;
      ...
      if tCli.Locate('ID', tTranCliente.Value, []) then begin
        tCli.Edit;
        tCliDebito.Value := TotalDeb(tCliID.Value);
        tCli.Post;
...
begin
  ...
  try
    ...
    on E: Exception do begin
      DebugMsg('ExcluiIME Exception: '+E.Message);
...
begin
  ...
  if (AjustaSaldoPost(IM.imProduto, IM.imDataHora, IM.imID, IM.SaldoPost)<0) and
     gConfig.NaoVenderAlemEstoque and (IM.imTipoTran=trEstVenda) then 
  begin
    Result := ncerrProdutoSemSaldo;
    Exit;
  end;
...
begin
  if tCliLimiteDebito.IsNull then
    Result := (tCliDebito.Value > gConfig.LimitePadraoDebito) else
    Result := (tCliDebito.Value > tCliLimiteDebito.Value);
end;
...
begin
  ...
  try
    ...
    while not tMovEst.Eof do begin
      ME.Itens.NewItem.LoadFromDataset(tMovEst);
      tMovEst.Next;
    end;
  finally
...
begin
  ...
  while not tPatro.Eof do begin
    SL.Add(
      tPatroNome.Value+sFldDelim+
      tPatroURLArea1.Value+sFldDelim+
      tPatroURLArea2.Value+sFldDelim+
...
begin
  ...
  if aChecaFmtTabs then
    ncEstruturaTabs.RestructureNexCafeDatabase(nxDB, 0, nil, RestProgressProc, nil);
...
procedure TDM.PermitirDownload(S: TncSessao; aPermitir: Boolean);
begin
  InitTran([tSessao], True);
  try
    S.AtualizaCache;
...
begin
  try
    ...
    begin
      InitTran([tConfig, tCaixa, tCli, tTempo, tTipoPass, tPacote, tPassaporte, tTran, tSessao, tITran, tEsp], False);
      try
        NumCX := NumCaixaAberto;
...
begin
  ...
  try
    ...
    while not tImp.Eof do begin
      aSessao.Impressoes.NewItem.LoadFromDataset(tImp);
      tImp.Next;
    end;
  finally
...
var 
  ...
  HC : TncHoraTarifa;
begin
  InitTran([tTipoAcesso, tHTar, tTarifa, tETar], True);
  try
    tTipoAcesso.Refresh;
    tHTar.Refresh;
    tTarifa.Refresh;
    ...
    for I := 0 to pred(gTiposAcesso.Count) do 
    with gTiposAcesso.Itens[I] do begin     
      for Dia := 1 to 7 do
      for H := 0 to 23 do begin
        if tHTar.FindKey([ID, Dia, H]) and
          tTarifa.FindKey([ID, tHTarCor.Value]) then 
...
begin
  ...
  if (aSessao.Cliente>0) and tCli.FindKey([aSessao.Cliente]) then begin
    ...
    aSessao.HP1 := tCliHP1.Value;
    aSessao.HP2 := tCliHP2.Value;
    aSessao.HP3 := tCliHP3.Value;
    aSessao.HP4 := tCliHP4.Value;
    aSessao.HP5 := tCliHP5.Value;
    aSessao.HP6 := tCliHP6.Value;
    aSessao.HP7 := tCliHP7.Value;
...
begin
  ...
  try
    ...
    tMovEst.First;
    while not tMovEst.Eof do begin
      aSessao.Vendas.NewItem.LoadFromDataset(tMovEst);
      tMovEst.Next;
    end;
  finally
    tMovEst.IndexName := SIndex;
...
begin
  ...
  try
    ...
    on E: Exception do begin
      DebugMsg('TDM.SalvaArqFundo - Exception: ' + E.Message);
      tConfig.Cancel;
      nxDB.Rollback;
      raise;
    end;
...
begin
  ...
  try
    try
      ...
      if S.UsernameLogin='anonimo' then begin
        if S.SenhaLogin <> 'anonimo' then begin
          Result := ncerrInfoLoginInvalida;
          Exit;
        end;
      end else  
...
begin
  ...
  Debitar := 0;
  MEAnt := nil;     
  NewTran := InitTran([tTran, tDebito, tITran, tMovEst, tAuxME, tCli, tCaixa, tSessao, tProduto], True);
  try
    try
      NumCx := NumCaixaAberto;
  
      if (NumCx=0) and (not aIgnoraCaixa) then begin
        Result := ncerrCaixaFechado;
        Exit;
      end;
      ...
      if not tTran.FindKey([ME.ID]) then  begin
        ...
        if tTranCaixa.Value<>NumCX then begin
          Result := ncerrCaixaDiferente;
          Exit;
        end;
      ...
      if (MEAnt<>nil) and (MEAnt.ValorDebitado>0) and
         (MEAnt.Cliente>0) and tCli.FindKey([MEAnt.Cliente]) then
      begin
        tCli.Edit;
        tCliDebito.Value := tCliDebito.Value - MEAnt.ValorDebitado;
        tCli.Post;

        if MEAnt.Cliente=ME.Cliente then
          Debitar := -1 * MEAnt.ValorDebitado;
      end;   

      if (ME<>nil) and (ME.ValorDebitado>0) and
         (ME.Cliente>0) and tCli.FindKey([ME.Cliente]) then
      begin
        tCli.Edit;
        tCliDebito.Value := tCliDebito.Value + ME.ValorDebitado;
        Debitar := Debitar + ME.ValorDebitado;
        tCli.Post;

{        if NewTran and (Debitar>0.009) and (tCliDebito.Value > LimiteDeb) then begin
          Result := ncerrLimiteDebitoExcedido;
          Exit;
        end;}
      end;   
    ...
    if NewTran and nxDB.InTransaction then 
      nxDB.Rollback;
    if MEAnt<>nil then 
      MEAnt.Free;
  end;
...
begin
  ...
  try
    ...
    case Ant.teTipo of
      ...
      tctPassaporte, tctCartaoTempo :
      if not tPassaporte.Locate('ID', Ant.teIDPacPass, []) then begin
        Result := ncerrItemInexistente;
        Exit;
      end else begin
        tPassaporte.Edit;
      ...
      if aCT.teTipo in [tctPassaporte, tctCartaoTempo] then begin
        if aCT.teTipo=tctPassaporte then begin
          ...
          if not tTipoPass.FindKey([aCT.teIDPacPass]) then 
            Raise Exception.Create('Passaporte não encontrado');
          ...
          try
            if not tPassaporte.FindKey([aCT.teSenha]) then
              Raise Exception.Create('Cartão de Tempo não encontrado');
          finally
            tPassaporte.IndexName := 'IID';
          end;
    ...
    if aCT.teCliente>0 then
      tTranNomeCliente.Value := tCli.Lookup('ID', aCT.teCliente, 'Nome') else
    ...
    begin
      tCli.Edit;
      tCliDebito.Value := TotalDeb(Ant.teCliente);
      tCli.Post;
    end;
...
function TDM.SalvaImpressao(Imp: TncImpressao; aSessoes: TncSessoes = nil): Integer;
var 
...
begin
  ...
  Debito := 0;
  try
    NewTran := InitTran([tImp, tTipoImp, tDebito, tCli, tTran, tITran, tCaixa, tSessao], True);
    try
      ...
      on E: Exception do begin
        DebugMsg('TDM.SalvaImpressao - Exception: ' + E.Message);
        CancelAndRollback;
        Raise;
      end;
    end;
...
begin
  Result := 0;
  InitTran([tDebito, tCli, tCaixa, tTran, tITran, tImp, tMovEst, tSessao, tTempo, tRec], True);
  try
    tDebito.CancelRange;
    ...
    for I := 0 to Itens.Count-1 do with Itens[I] do begin
      ...
      if not tDebito.FindKey([idTipoItem, idItemID, aDeb.Cliente]) then begin
        Result := ncerrItemInexistente;
        Exit;
      end;
      ...
      case idTipoItem of
        ...
        itTempo : 
        if tTempo.Locate('ID', idItemID, []) then begin
          tTempo.Edit;
    ...
    tCli.Edit;
    tCliDebito.Value := TotalDeb(aDeb.Cliente);
    tCli.Post;
...
procedure TDM.SalvaPaginasImpressas(aMaq: Word; Maq: TncMaquina;
...
begin
  ...
  try
    InitTran([tImp, tTipoImp, tTran, tITran, tSessao, tCli, tDebito], True);
    try
      Imp.impDataHora := Now;
      if Maq<>nil then
        Imp.impComputador := Maq.ComputerName else
