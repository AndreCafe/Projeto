...
uses
  ...
  kbmMWTCPIPIndyMessagingClientTransport, LMDGlobalHotKey, AppEvnts;
...
uses 
  ...
  ncaFrmTempo,
  ncafbPesqCli, 
  ncafbAvisos, ncafbFiltroWeb, ncVersoes, ncDMServ, ncClassesBase,
  ncFrmSuporteRem;
...
begin
  ...
  Result := Result[1] + '.' + Result[2] + '.' + Result[3] + Copy(Dados.VI.FileVersion, 6, 20);
end;
...
begin
  Maq := 0;
  Porta := 0;
  if TFrmSuporte.Create(nil).ObterSuporte(Maq, Porta, Dados.CM.Ativo, Dados.CM.Maquinas) then 
    if Maq=0 then begin
      with Dados.CM.UA do 
      S := Username + ': ' + Nome;
      ChamaSuporte(S, VersaoStr);
    end else
      Dados.CM.SuporteRem(Maq, Porta);    
end;
...
begin
  ...
  S := 'Versão ' + S[1] + '.' + S[2] + '.' + S[3] + Copy(Dados.VI.FileVersion, 6, 20);
  AboutDlg.Version := S;
...
begin
  RegistraForms;  
  FChat := nil;
  AboutDlg.Version := 'Versão: ' + Dados.VI.FileVersion;
  cmVerCod.Caption := '|  '+AboutDlg.Version + '  |  Cod.Loja: ' + Dados.CM.Config.CodLoja + '  |';
end;
