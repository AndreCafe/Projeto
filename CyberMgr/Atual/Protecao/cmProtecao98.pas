...
procedure TCMProtege98.RegOpenEx(AHKey: HKey; S: String);
begin
...
procedure TCMProtege98.RegWriteEx(AHKey: HKEY; Key, Nome: String; Valor: Boolean);
begin
  if Valor then
    RegWriteDWEx(AHKEY, Key, Nome, 1)
  else
    RegWriteDWEx(AHKEY, Key, Nome, 0)
end;

procedure TCMProtege98.RegWriteDWEx(AHKey: HKEY; Key, Nome: String; Valor: DWORD);
begin
  try
    FRegistry.RootKey := AHKEY;
    FRegistry.Access := KEY_ALL_ACCESS;
...
procedure TCMProtege98.RegDeleteEx(AHKEY: HKEY; Key, Nome: String);
begin
  try
    FRegistry.RootKey := AHKEY;
    FRegistry.Access := KEY_ALL_ACCESS;
...
procedure TCMProtege98.RegWriteStringEx(AHKey: HKEY; Key, Nome: String; Valor: String);
begin
  try
    FRegistry.RootKey := AHKEY;
    FRegistry.Access := KEY_ALL_ACCESS;
...
begin
  ...
  RegDelete('\Software\Microsoft\Windows NT\CurrentVersion\AppCompatFlags\Layers', 'C:\cybermgr\CMGuard.exe');
  try
    inherited;
    AplicaProtStartMenu;
...
procedure TCMProtege98.DoHideDrives(ADrives: String);
var 
  Valor: Cardinal;
  C : Char;
begin
...
procedure TCMProtege98.RegWriteString(Key, Nome, Valor: String);
begin
  RegWriteStringEx(HKEY_CURRENT_USER, Key, Nome, Valor);
  RegWriteStringEx(HKEY_LOCAL_MACHINE, Key, Nome, Valor);
end;

procedure TCMProtege98._NoNetwork(No: Boolean);
begin
  ...
  if No then
    RegWriteDWEx(HKEY_CLASSES_ROOT, Reg_ShellFolder, 'Attributes', 20180000)
  else
