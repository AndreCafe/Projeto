...
type
  TForm16 = class(TForm)
    ...
    tFsEscola: TWideStringField;
    tFsHorario1: TWideStringField;
    tFsHorario2: TWideStringField;
  ...
  while not TF.eof do begin
    ...
    if not SameText(tFSNome.Value, 'Ticket') then begin
      ...
      if tTempo.Locate('ObjUsuario', tFObjUsuario.Value, []) then begin
        ...
        qMov.Active := False;
{        qMov.Parameters[0].Value := tFObjUsuario.Value;
        qMov.Parameters[1].Value := tTempodInicio.Value;}
        qMov.SQL.Text := 'Select * from Movimentacao where (objUsuario = ' + tFObjUsuario.AsString + ')';
        qMov.Open;
        qMov.First;
        while not qMov.Eof do begin
//         if (qMovdEntrada.Value >= tTempodInicio.Value) then 
