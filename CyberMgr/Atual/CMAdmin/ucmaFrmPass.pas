...
type
  TFrmPassaporte = class(TForm)
    ...
    Label1: TLabel;
    panExpirar: TLMDSimplePanel;
    btnExpirar: TPngSpeedButton;
    ...
    procedure btnExpirarClick(Sender: TObject);
...
begin
  ...
  if FPassaporte^.pcExpirou then
    panExpirar.Visible := False;
  with FPassaporte^ do
...
var I : Integer;
begin
  I := tHistTransacao.Value;
  if Dados.tbAcesso.Locate('Numero', I, []) then
...
procedure TFrmPassaporte.btnExpirarClick(Sender: TObject);
begin
  if SimNaoH('Deseja realmente forçar a expiração desse passaporte?', Handle) then 
