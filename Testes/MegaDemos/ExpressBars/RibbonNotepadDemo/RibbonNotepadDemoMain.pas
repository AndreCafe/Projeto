...
uses
  ...
  ImgList, ActnList, cxLookAndFeels, dxStatusBar, cxGraphics, dxRibbonForm,
  dxRibbon, cxClasses, Menus, dxRibbonStatusBar, cxStyles, dxRibbonGallery,
  dxOffice11, cxGeometry, dxRibbonGalleryFilterEd, cxLookAndFeelPainters,
  cxContainer, cxEdit, dxRibbonSkins, dxRibbonBackstageView, dxBarApplicationMenu,
  RibbonNotepadDemoOptions, dxSkinChooserGallery, dxSkinsdxRibbonPainter,
...
type
  ...
  TRibbonDemoMainForm = class(TdxRibbonForm)
    ...
    SkinChooser: TdxSkinChooserGalleryItem;
    ...
    procedure SkinChooserSkinChange(Sender: TObject; const ASkinName: string);
  private
    ...
    procedure SkinChooserSelectSkinName(const AName: string);
...
uses
  RichEdit, ClipBrd, RibbonNotepadDemoGallerySetup, Math, dxGDIPlusAPI, dxDemoUtils, dxAboutDemo;
...
procedure TRibbonDemoMainForm.SkinChooserSelectSkinName(const AName: string);

  function FindSkinItemInGroup(AGroup: TdxRibbonGalleryGroup; const AName: string): TdxSkinChooserGalleryGroupItem;
  var
    I: Integer;
  begin
    Result := nil;
    for I := 0 to AGroup.Items.Count - 1 do
      ...
      begin
        Result := TdxSkinChooserGalleryGroupItem(AGroup.Items[I]);
        Break;
      end;
  end;

  function FindSkinItem(const AName: string): TdxSkinChooserGalleryGroupItem;
  var
    I: Integer;
  begin
    Result := nil;
    for I := 0 to SkinChooser.GalleryGroups.Count - 1 do
    begin
      Result := FindSkinItemInGroup(SkinChooser.GalleryGroups.Items[I], AName);
      if Result <> nil then Break;
    end;
  end;

begin
  SkinChooser.SelectedGroupItem := FindSkinItem(AName);
end;
...
procedure TRibbonDemoMainForm.SkinChooserSkinChange(
...
procedure TRibbonDemoMainForm.RibbonHelpButtonClick(Sender: TdxCustomRibbon);
begin
  dxShowAboutForm;
end;
