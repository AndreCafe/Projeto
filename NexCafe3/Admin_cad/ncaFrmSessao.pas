...
uses ncaDM, ncaFrmPri, ncIDRecursos, ufmImagens, ncCredTempo, ncaFrmTempo,
  ...
  case mtTranTipo.Value of
    trEstVenda  : begin
      ...
      try
        ...
        if TFrmVendaProd.Create(nil).Editar(IME, False, True, (not IME.imCancelado) and (IME.imCaixa=NumAberto)) then begin
    ...
    trCredTempo : begin
      ...
      if TFrmTempo.Create(nil).Editar(False, (not CT.teCancelado) and (CT.teCaixa=NumAberto), @DT, CT.teTran) 
      then begin
...
procedure TFrmSessao.vgHExit(Sender: TObject);
begin
//  MostraDebito;
end;

procedure TFrmSessao.vgHItemChanged(Sender: TObject; AOldRow: TcxCustomRow;
  AOldCellIndex: Integer);
begin
//  MostraDebito;
end;
...
procedure TFrmSessao.MostraDebito;
begin
...
begin
  ...
  try
    ...
    FSessao.Vendas.NewItem.AssignFrom(IME);
    ...
    Tran.LoadFromDataset(mtTran);
    mtTranObjTran.Value := Integer(Tran);
    mtTranObjItem.Value := Integer(IME);
    mtTran.Post;
    RefreshTotais;
  finally
    IME.Free;
