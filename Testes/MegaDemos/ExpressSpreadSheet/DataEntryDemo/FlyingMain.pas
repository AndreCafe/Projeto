...
uses
  ...
  ComCtrls, Jpeg, Menus, cxssPainters, cxGraphics, cxLookAndFeels,
  cxLookAndFeelPainters, dxBar, cxClasses;
...
type
  TFlyingMainForm = class(TForm)
    ...
    mmExit: TdxBarButton;
    File1: TdxBarSubItem;
    moToday: TdxBarButton;
    moFuelTabs: TdxBarButton;
    mmOptions: TdxBarSubItem;
    siHelp: TdxBarSubItem;
    mmAircraft: TdxBarSubItem;
...
uses
  FlyingSeatCalc, dxDemoUtils, dxAboutDemo;
...
begin
  ...
  Rs := TResourceStream.Create(hInstance, 'DEFAULTSHEET', RT_RCDATA);                 // Load the Spreadsheet into a resource stream
  try
  ...
  begin
    ...
    AButton.Caption := cxSpread.Pages[I].Caption;
    AButton.Tag := I;
    AButton.OnClick := moSelectPlaneClick;
    mmAircraft.ItemLinks.Add(AButton);
  end;
...
begin
  If cxSpread.ActivePage <> (Sender as TComponent).Tag then
  begin
    cxSpread.ActivePage := (Sender as TComponent).Tag;        // Get the sheet they selected
