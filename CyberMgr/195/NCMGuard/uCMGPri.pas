...
uses
  ...
  am2000menuitem, am2000utils, am2000popupmenu, am2000, lmdcont;
...
begin
  with CM do 
  if Status=stCliUsando then begin
...
//    MostraPreco := (slRecursos.Values['Preco']='T');
    ...
    if (Maq<>nil) and (Maq.Acesso>0) and (not Maq.AguardaPagto) then begin
      ...
      if TA=nil then begin
        ...
        if Config.MostraPrePagoDec and (Tempo<=Maq.LimiteTempo) and (Maq.CreditoTotal<10000) then
...
//        btnConfigurar.Enabled := (M=nil) or ((U<>nil) and CM.Permitido(U, reMaquinas, taMaqConfigurar));
      end;  
    end;
...
begin
  ...
  if Ativo then begin
    ...
    ShowMessage('Solicite ao atendente o encerramento do seu acesso pois esta'+
                ' máquina encontra-se desconectada do servidor no momento');     
end;
