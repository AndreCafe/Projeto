...
begin
  if (edValorI.Value>0) and (not cbSemValorMin.Checked) and (edTempoI.Value<1) then begin
    edTempoI.SetFocus;
    Raise ENexCafe.Create('É necessário informar a quantidade de minutos que o cliente pode utilizar pagando o valor inicial');
  end;
  if edValor1h.Value < edValorI.Value then
    Raise ENexCafe.Create('O valor inicial não pode ser maior que o valor cobrado por 1 hora de uso');
...
begin
  if Trim(edNome.Text)='' then
    Raise ENexCafe.Create('É necessário informar um nome para identificar essa tarifa');

  if (edCor.ColorValue=clSilver) or (edCor.ColorValue=clNone) or (edCor.ColorValue=clWhite) or (edCor.ColorValue=clBlack) then
    Raise ENexCafe.Create('É necessário selecionar uma cor para identificar essa tarifa');
  ...
  if (FNovo or (edCor.ColorValue<>FTarifa.Cor)) and (gTarifas.PorCor[edCor.ColorValue]<>nil) then
    Raise ENexCafe.Create('Já foi cadastrado uma tarifa com essa cor');
...
begin
  ...
  edArr.Text := IntToStr(aTarifa.Arredondar) + ' centavos';
  ...
  if FDivTipo=tdtPorTempo then
    btnDiv.Text := IntToStr(FDivQtd)+' minuto(s)' else
...
begin
  ...
  if edMaisHoras.Enabled then
    lbHoras.Caption := 'horas' else
    lbHoras.Caption := 'hora';
...
begin
  ...
  edArr.Text := IntToStr(FArr) + ' centavos';
  Load;
  grid.Setfocus;
end;
...
begin
  ...
  FDivQtd := btnDiv.Tag;
  btnDiv.Text := IntToStr(FDivQtd) + ' minuto';
...
begin
  ...
  if (not cbSemValorMin.Checked) and (FValorI>0) and (FTempoI<1) then
    Raise ENexCafe.Create('O tempo inicial tem que ser informado');
end;
