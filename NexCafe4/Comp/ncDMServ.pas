...
type
  ...
  public
    ...
    function SalvaImpressao(Imp: TncImpressao; J: TncJob; aSessoes: TncSessoes = nil): Integer;
...
uses ncTableDefs, ncVersoes, ncDebug, ncMsgCom;
  ...
  try
    ...
    if IM._Operacao=osAlterar then begin
      if Achou then begin
        ...
        if tProduto.FindKey([IM.imProduto]) and (not tProdutoNaoControlaEstoque.Value) then
...
begin
  ...
  case S.Impressoes[k]._Operacao of
    ...
    osAlterar  : begin
      ...
      Result := SalvaImpressao(S.Impressoes[k], nil);
      if Result<>0 then Exit;
    end;
...
function TDM.SalvaImpressao(Imp: TncImpressao; J: TncJob; aSessoes: TncSessoes = nil): Integer;
var 
...
begin
  ...
  Debito := 0;
  try
    NewTran := InitTran([tImp, tPM, tTipoImp, tDebito, tCli, tTran, tITran, tCaixa, tSessao], True);
    try
...
begin
  if J.Liberacao=jlPendente then begin
    ...
    if (aControl=jc_resume) and 
        gConfig.PMPausaAutomatica and 
        (gConfig.ControlaImp in [ciMonitorar, ciRegistrar]) and
        (J.ImpID=0) 
    then begin
      if ((J.Maquina>0) or gConfig.DetectarImpServ) and (J.PaginasReg>0) then 
  ...
  try
    if not tPM.FindKey([J.PrinterIndex, J.JobID]) then begin
      ...
      begin
        tPM.Delete;
        nxDB.Commit;
        if (not (J.Status in [jsCompleted])) then Exit;
