...
type
  ...
  public
    ...
    procedure AjustaOperacao(B: TncItensMovEst);
...
constructor TncItemMovEst.Create;
begin
  inherited;
  Limpa;
end;
...
begin
  Result := 
    IntToStr(imID) + sFldDelim +
    IntToStr(imTran) + sFldDelim +
    IntToStr(imProduto) + sFldDelim +
    FloatParaStr(imQuant) + sFldDelim +
    FloatParaStr(imUnitario) + sFldDelim +
    FloatParaStr(imTotal) + sFldDelim +
    FloatParaStr(imCustoU) + sFldDelim +
    IntToStr(imItem) + sFldDelim +
    FloatParaStr(imDesconto) + sFldDelim +
    FloatParaStr(imPago) + sFldDelim +
    GetDTStr(imDataHora) + sFldDelim +
    BoolStr[imCancelado] + sFldDelim +
    FloatParaStr(imEstoqueAnt) + sFldDelim +
    IntToStr(imTipoTran) + sFldDelim +
    IntToStr(imCliente) + sFldDelim +
    IntToStr(imSessao) + sFldDelim +
    IntToStr(imCaixa) + sFldDelim +
    imCategoria + sFldDelim +
    BoolStr[imNaoControlaEstoque] + sFldDelim +
    IntToStr(imITran) + sFldDelim +
    BoolStr[imFidResgate] + sFldDelim +
    FloatParaStr(imFidPontos) + sFldDelim +
    BoolStr[_Recibo] + sFldDelim + 
...
begin
  imID := tMovEst.FieldByName('ID').AsInteger;
  imTran := tMovEst.FieldByName('Tran').AsInteger;
  imTipoTran := tMovEst.FieldByName('TipoTran').AsInteger;
  imProduto := tMovEst.FieldByName('Produto').AsInteger;
  imQuant := tMovEst.FieldByName('Quant').AsFloat;
  imUnitario := tMovEst.FieldByName('Unitario').AsCurrency;
  imTotal := tMovEst.FieldByName('Total').AsCurrency;
  imCustoU := tMovEst.FieldByName('CustoU').AsCurrency;
  imItem := tMovEst.FieldByName('Item').AsInteger;
  imDesconto := tMovEst.FieldByName('Desconto').AsCurrency;
  imPago := tMovEst.FieldByName('Pago').AsCurrency;
  imDataHora := tMovEst.FieldByName('DataHora').AsDateTime;
  imCancelado := tMovEst.FieldByName('Cancelado').AsBoolean;
  imEstoqueAnt := tMovEst.FieldByName('EstoqueAnt').AsFloat;
  imCliente := tMovEst.FieldByName('Cliente').AsInteger;
  imSessao := tMovEst.FieldByName('Sessao').AsInteger;
  imCaixa := tMovEst.FieldByName('Caixa').AsInteger;
  imCategoria := tMovEst.FieldByName('Categoria').AsString;
  imNaoControlaEstoque := tMovEst.FieldByName('NaoControlaEstoque').AsBoolean;
  imITran := tMovEst.FieldByName('ITran').AsInteger;
  imFidResgate := tMovEst.FieldByName('FidResgate').AsBoolean;
  imFidPontos := tMovEst.FieldByName('FidPontos').AsFloat;
  _Recibo := False;
  _Operacao := osNenhuma;
end;
...
begin
  D.FieldByName('Tran').AsInteger := imTran;
  D.FieldByName('Produto').AsInteger := imProduto;
  D.FieldByName('Quant').AsFloat := imQuant;
  D.FieldByName('CustoU').AsCurrency := imCustoU;
  if imFidResgate then begin
    D.FieldByName('Unitario').Clear;
    D.FieldByName('Total').Clear;
    D.FieldByName('Desconto').Clear;
    D.FieldByName('Pago').Clear;
    D.FieldByName('FidPontos').AsFloat := imFidPontos;
  end else begin
    D.FieldByName('Unitario').AsCurrency := imUnitario;
    D.FieldByName('Total').AsCurrency := imTotal;
    D.FieldByName('Desconto').AsCurrency := imDesconto;
    D.FieldByName('Pago').AsCurrency := imPago;
    D.FieldByName('FidPontos').Clear;
  end;
  D.FieldByName('Item').AsInteger := imItem;
  D.FieldByName('DataHora').AsDateTime := imDataHora;
  D.FieldByName('Entrada').AsBoolean := Entrada;
  D.FieldByName('Cancelado').AsBoolean := imCancelado;
  D.FieldByName('EstoqueAnt').AsFloat := imEstoqueAnt;
  D.FieldByName('TipoTran').AsInteger := imTipoTran;
  D.FieldByName('Cliente').AsInteger := imCliente;
  D.FieldByName('Sessao').AsInteger := imSessao;
  D.FieldByName('Caixa').AsInteger := imCaixa;
  D.FieldByName('Categoria').AsString := imCategoria;
  D.FieldByName('NaoControlaEstoque').AsBoolean := imNaoControlaEstoque;
  D.FieldByName('ITran').AsInteger := imITran;
  D.FieldByName('FidResgate').AsBoolean := imFidResgate;
end;
...
begin
  ...
  D.FieldByName('ItemID').AsInteger := imID;
  D.FieldByname('ItemPos').AsInteger := imItem;
  D.FieldByName('Cancelado').AsBoolean := imCancelado;
  D.FieldByName('TipoTran').AsInteger := imTipoTran;
  D.FieldByName('Sessao').AsInteger := imSessao;
  D.FieldByName('Cliente').AsInteger := imCliente;
  ...
  if imFidResgate then begin
    ...
    D.FieldByName('Desconto').Clear;
    D.FieldByName('Pago').Clear;
    D.FieldByName('FidFator').AsInteger := -1;
    D.FieldByName('FidPontos').AsFloat := imFidPontos;
  end else begin
    D.FieldByName('Total').AsCurrency := imTotal;
    D.FieldByName('Desconto').AsCurrency := imDesconto;
    D.FieldByName('Pago').AsCurrency := imPago;
    D.FieldByName('FidFator').Clear;
    D.FieldByName('FidPontos').Clear;
  end;
end;
...
procedure TncItensMovEst.AjustaOperacao(B: TncItensMovEst);
var 
  I : Integer;
  IM : TncItemMovEst;
begin
...
constructor TncItensMovEst.Create;
begin
  FItens := TList.Create;
end;
...
procedure TncItensMovEst.SetString(const Value: String);
var 
  SL : TStrings;
  I  : Integer;
begin
...
begin
  if Cancelado or (Sessao<>0) or FidResgate then
    Result := 0 else
    Result := Total - Desconto - Pago;
end;
