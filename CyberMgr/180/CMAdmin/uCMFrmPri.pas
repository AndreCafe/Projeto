...
uses 
  ...
  cmIDRecursos,
  dxGridMenus, 
  uCMFrmGrupo, uCMFrmVendaPac, uCMFrmCaixa, uCMFrmLancExtra,
  uCMfbgTipoTarifas, uCMFrmHoraCor, uCMFrmContato, uCMFrmChave,
...
begin
  cmTotPer.Enabled := Permitido(reCaixas, taCaiVerAntesOutros);
  if NumAberto > 0 then begin
    cmCaixa.Caption := 'Caixa';
    cmLancarEntrada.Enabled := Permitido(reCaixas, taCaiSuprimento);
    cmLancarSaida.Enabled := Permitido(reCaixas, taCaiSangria);
    cmListaCaixas.Enabled := Permitido(reCaixas, taCaiVerAntes) or Permitido(reCaixas, taCaiVerAntesOutros);
...
begin
  ...
  cmCadastroPac.Enabled := Permitido(reConfig, taCfgPrecos);
...
begin
  TFrmVendaPass.Create(Self).Novo(Dados.tbAcesso);
end;
