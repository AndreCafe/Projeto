...
uses
  ...
  LMDPNGImage, ComCtrls, cxTextEdit, cxMaskEdit, Buttons, PngSpeedButton;
...
begin
  ...
  try
    ...
    for I := 0 to SL.Count-1 do begin
      S := SL[i];
      if SameText(Copy(S, 1, 4), 'www.') then Delete(S, 1, 4);
        if (not Tab.FindKey([S])) and (not Tab.FindKey(['www.'+S])) then
      begin
        Inc(C);
        Tab.Insert;
        TabURL.Value := '#imp#'+S;
    ...
    ShowMessage('Fim de importação. '+IntToStr(C)+' sites importados');
  finally
    DS.DataSet := Tab;
    Tab.EnableControls;
    PB.Visible := False;
