...
resourcestring
  SJ�ExisteUmaTransf�nciaDeArquivoE = 'J� existe uma transf�ncia de arquivo em andamento';
  STClienteNexCafe�PrecisoInformarO = 'TClienteNexCafe: � preciso informar o servidor do NexCaf�';
...
begin
  if FTransferindoArq<>taNenhum then
    Raise ENexCafe.Create(SJ�ExisteUmaTransf�nciaDeArquivoE);
  if aDesktop then
  ...
  if Valor then begin
    ...
    try
      if FServidor=nil then 
        Raise ENexCafe.Create(STClienteNexCafe�PrecisoInformarO);
      ...
      if (not FServidor.Ativo) then begin
        ...
        DebugMsg('Conex�o realizada');   // do not localize
      end else
        DebugMsg('Servidor j� estava ativo.'); // do not localize
        
  
      if FAutoAtualizar and (not LadoServidor) then begin
        DebugMsg('Obter vers�o do NexGuard dispon�vel no servidor'); // do not localize
        ChecaErro(FServidor.ObtemVersaoGuard(Ver));
        DebugMsg('Vers�o dispon�vel no servidor x'+IntToStr(Ver)+' vers�o atual no client ' + FVersao); // do not localize
        if (Ver>0) and (Ver<>GetVersionBuild(FVersao)) then begin
          DebugMsg('Baixando nova vers�o'); // do not localize
      ...
      DebugMsg('Login da m�quina OK'); // do not localize
      ...
      try
        ...
        try
          ...
          DebugMsg('Obtem lista de m�quinas ...'); // do not localize
          ChecaErro(FServidor.ObtemStreamListaObj(FHandle, tcMaquina, S));
          DebugMsg('Lista de m�quinas OK'); // do not localize
          S.Position := 0;
