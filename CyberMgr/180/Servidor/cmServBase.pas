...
type  
  ...
  public  
    ...
    function Permitido(U: TCMUsuario; Recurso, TipoAcesso: Integer): Boolean;
    function TodosPerm(Recurso, TipoAcesso: Integer): Boolean;
...
uses cmChecaBD, LMDStarter, cmConverteArq, cmAuxPassaporte;

{ TCMServidor }

function PermitidoDir(Direitos: String; Recurso, TipoAcesso: Integer): Boolean;
var Valor: String;
begin
    ...
    try
      ...
      if (Status=jsCompleted) and (Pages>0) then begin
        ...
        begin
          ...
          FTMovEst.FieldByName('Quant').AsFloat       := Pages;
          FTMovEst.FieldByName('Unitario').AsFloat    := FTProduto.FieldByName('Preco').AsFloat;
          FTMovEst.FieldByName('Total').AsFloat       := (FTProduto.FieldByName('Preco').AsFloat * Pages);
  ...
  try
    ...
    try
      if aEntrar then begin
        ...
        if not Permitido(Usuario, reMaquinas, taMaqModoManutencao) then begin
...
function TCMServidor.Permitido(U: TCMUsuario; Recurso,
  TipoAcesso: Integer): Boolean;
var
  Dir: String;
  I : Integer;  
begin
  if not U.Admin then begin
    ...
    Result := PermitidoDir(Dir, Recurso, TipoAcesso);
  end else
    Result := True;
end;

function TCMServidor.TodosPerm(Recurso, TipoAcesso: Integer): Boolean;
var I : Integer;
begin
  Result := False;
  for I := 0 to Usuarios.Count-1 do
  if not Permitido(Usuarios[I], Recurso, TipoAcesso) then
    Exit;
  Result := True;
end;
    ...
    try
      ...
      try
        ...
        if aContato > 0 then begin
          ...
          LePassaportesCli(FTPassaporte, aTipoAcesso, aContato, Maq.Passaportes)
        end else
        ...
        FDB.StartTransactionWith([FTTran, FTMaquina, FTPassaporte]);
        try
          FTTran.Append;
