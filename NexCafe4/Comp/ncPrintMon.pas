...
Type
  ...
  protected
    ...
    Procedure JobNotification( const aJobinformation: TncJob );
...
var Erro: Integer;    
  Begin
    if (aJobInformation.Status=jsNew) and gConfig.PMPausaAutomatica and gNaoPausar.Pausar(aJobInformation.Computer) then begin
      aJobInformation.Pausou := True;
      if SetJob(aJobInformation.PrtHandle, aJobInformation.JobID, 0, nil, JOB_CONTROL_PAUSE) then 
      begin
        aJobInformation.Pausou := True;
        DebugMsg('TncPrintersMonitor.JobNotification - SetJob - JobID: ' + IntToStr(aJobInformation.JobId) + ' - JOB_CONTROL_PAUSE OK');
      end else 
        DebugMsg('TncPrintersMonitor.JobNotification - SetJob - JobID: ' + IntToStr(aJobInformation.JobId) + ' - JOB_CONTROL_PAUSE ERROR: ' + IntToStr(GetlastError));
    end else begin
      aJobInformation.Pausou := False;
      if aJobInformation.Status=jsNew then
