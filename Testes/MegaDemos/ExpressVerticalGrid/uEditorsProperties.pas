...
uses
  dxFrames, FrameIDs, cxVerticalGridDemoUtils, TypInfo, maindata,
  uStrsConst, cxLookupGrid, dxCustomHint;
...
begin
  ...
  for I := 0 to ACount - 1 do
     if APropList^[I].PropType^.Kind = tkEnumeration then
       ACombo.Properties.Items.AddCheckItem(string(APropList^[I].Name));
end;
...
begin
  ...
  if IsPictureAssigned(fmPreview.Image.Picture) then
  begin
...
initialization
  dxFrameManager.RegisterFrame(EditorsPropertiesFrameID, TframeEditorsProperties,
    EditorPropertiesFrameName, EditorsPropertiesImageIndex, EditorsPropertiesImageIndex, EditorsSideBarGroupIndex);
