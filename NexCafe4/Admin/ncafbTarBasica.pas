...
uses
  ...
  cxContainer, cxLabel, LMDPNGImage, cxCheckBox;
...
begin
  ...
  if s='sapato' then Exit;
  

  T := gTarifas.PorCor[TabCor.Value];
  if T<>nil then
    TabTarifasPorHora.Value :=
      '1h = ' + Trim(FloatToStrF(T.ValorHora(1), ffCurrency, 10, 2)) +
      '  2h = ' + Trim(FloatToStrF(T.ValorHora(2), ffCurrency, 10, 2)) +
      '  3h = ' + Trim(FloatToStrF(T.ValorHora(3), ffCurrency, 10, 2));
end;
...
begin
  ...
  if Tab.RecordCount=1 then begin
    ShowMessage('Não é possível apagar todas tarifas');
    Exit;
  end;
  if TabCor.Value=gConfig.TarifaPadrao then begin
    ShowMessage('Essa é a tarifa padrão. Não é permitido apagá-la');
    Exit;
  end;
  T := gTarifas.PorCor[TabCor.Value];
  if T<>nil then
  if SimNaoH('Deseja realmente apagar o tarifa ' + TabDescricao.Value + '?', Handle) then
  begin
    Dados.CM.ApagaObj(T);
    Tab.Refresh;
  end;
