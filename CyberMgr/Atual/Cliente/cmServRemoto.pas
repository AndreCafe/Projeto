...
function TCMServidorRemoto.Download(aFonte, aDestino: String): Integer;
var I: Integer;
begin
  I := 0;
...
function TCMServidorRemoto.EnviarMsg(const aDe, aPara: Integer;
...
begin
  ...
  try
    ...
    SL.Add(aTexto);
    SL.SaveToStream(S);
    Result := FCliCom.RequestData(cmnmEnviaChat, S, S.Size, nmdStream, nil, 0, nmdByteArray);
  finally
    SL.Free;
    S.Free;
  end;
...
procedure TCMServidorRemoto.nmChatEv(var Msg: TCSCMessage);
var S: TMemoryStream;
begin
  S := TMemoryStream.Create;
  S.LoadFromStream(TStream(Msg.dmData));
...
function TCMServidorRemoto.ArqFundoEnviado(NomeArq: String): Integer;
var Request : TnmNomeArq;
begin
  Request.nmNomeArq := NomeArq;
  Result := FCliCom.RequestData(cmnmArqFundoEnviado, @Request, SizeOf(Request), 
                                nmdByteArray, nil, 0, nmdByteArray);
end;

function TCMServidorRemoto.SalvaSenhaCli(Codigo: Integer;
  Senha: String): Integer;
var Request : TnmSenhaCli;
begin
  Request.nmCodigo := Codigo;
  Request.nmSenha  := Senha;
  Result := FCliCom.RequestData(cmnmSalvaSenhaCli, @Request, SizeOf(Request), 
                                nmdByteArray, nil, 0, nmdByteArray);
end;
...
function TCMServidorRemoto.Upload(aFonte, aDestino: String): Integer;
var I: Integer;
begin
  I := 0;
...
function TCMServidorRemoto.LimpaFundo(Desktop: Boolean): Integer;
var Req : TnmLimpaFundoReq;
begin
  Req.nmDesktop := Desktop;
  Result := FCliCom.RequestData(cmnmLimpaFundo,  @Req, SizeOf(Req), nmdByteArray,
                                nil, 0, nmdByteArray);
end;
...
function TCMServidorRemoto.ObtemSenhaCli(Codigo: Integer;
...
begin
  ...
  Reply.nmCodigo := 0;
  Reply.nmSenha := '';
  Result := FCliCom.RequestData(cmnmObtemSenhaCli, @Request, SizeOf(Request), 
                                nmdByteArray, @Reply, SizeOf(Reply), nmdByteArray);
  if Result=0 then
    Senha := Reply.nmSenha;                               
end;

function TCMServidorRemoto.ObtemStreamAvisos(S: TStream): Integer;
begin
  Result := FCliCom.RequestData(cmnmAvisos, nil, 0, nmdByteArray, Pointer(S), 0, nmdStream);
  S.Position := 0;
end;
...
function TCMServidorRemoto.ObtemPastaServ(var NomePastaServ: String): Integer;
var Reply: TnmNomeArq;
begin
  Reply.nmNomeArq := '';
  Result := FCliCom.RequestData(cmnmObtemPastaServ, nil, 0, nmdByteArray, @Reply, SizeOf(Reply), nmdByteArray);
...
function TCMServidorRemoto.AdicionaPassaporte(aMaq: Byte;
  aSenha: String): Integer;
var Req: TnmAdicionaPassaporteReq;
begin
  Req.nmSenha := aSenha;
  Req.nmMaq := aMaq;
  Result := FCliCom.RequestData(cmnmAdicionaPassaporte, @Req, SizeOf(Req), nmdByteArray, nil, 0, nmdByteArray);
end;

function TCMServidorRemoto.RegistraPaginasImpressas(aMaq: Byte;
...
begin
  ...
  Req.nmImp := aImp;
  Req.nmDoc := aDoc;
  Result := FCliCom.RequestData(cmnmPaginasImpressas, @Req, SizeOf(Req), nmdByteArray, nil, 0, nmdByteArray);
end;
