...
uses
  ...
  cxRadioGroup, Menus, cxLookAndFeelPainters, cxButtons;
...
begin
  if rbEmail.Checked then begin
    if Trim(edEmail.Text)='' then
      raise exception.Create('É necessário informar o seu endereço de e-mail');
    dmLic := TdmAtualizaLic.Create(Self);
    try
      if dmLic.ObtemCodLojaPorEmail(Trim(edEmail.Text)) then begin
        ...
        raise exception.create('Não foi encontrado um registro de loja com o e-mail informado');
    finally
      dmLic.Free;
    end;
  end else begin
    ...
    try
      ...
      S := dmLic.ObtemCodLojaPorEquip(S);
      if S>'' then begin
        ShowMessage('As informações de registros foram enviadas para: '+sLineBreak+sLineBreak+S);
        ShowMessage('Caso não receba o e-mail em sua caixa de entrada confira se não está em sua caixa de SPAM ou Lixo Eletrônico');
      end else
        raise exception.create('Não foi encontrado um registro anterior do NexCafé para esse computador');
    finally
      dmLic.Free;
    end;
  end;
