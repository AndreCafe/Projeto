...
uses ucmaPri, ucmaDM, ucmaFrmCorPreco, cmIDRecursos;
...
begin
  ...
  try
    ...
    while not tAux.Eof do begin
      cmTiposAcesso.Items.AddObject(tAuxNome.Value, TObject(tAuxCodigo.Value));
      tAux.Next;
    end;
    if Ant<>-1 then begin
...
begin
  with Dados do begin
    DB.StartTransactionWith([tbCor, tbHoraCor]);
    try
      SalvaCores;
      SalvaHoraCor;
      SalvaEtapas;
...
begin
  with Dados do begin
    tbCor.SetRange([TipoAcesso], [TipoAcesso]);
...
begin
  ...
  try
    TempoAnt := 0;
    ValorAnt := 0;
    tbEtapas.SetRange([TipoAcesso], [TipoAcesso]);
    tbEtapas.First;
    while not tbEtapas.Eof do begin
      CorAnt := tbEtapasCor.Value;
      mtEtapas.Insert;
      TransfDados(tbEtapas, mtEtapas);
      mtEtapasTempo.Value := mtEtapasTempo.Value + TempoAnt;
      mtEtapasValor.Value := mtEtapasValor.Value + ValorAnt;
      TempoAnt := TempoAnt + tbEtapasTempo.Value;
      ValorAnt := ValorAnt + tbEtapasValor.Value;
      mtEtapas.Post;
      tbEtapas.Next;
      if (CorAnt <> tbEtapasCor.Value) then begin
        TempoAnt := 0;
        ValorAnt := 0;
      end;
    end;
  finally
    tbEtapas.CancelRange;
  ...
  with Dados do begin
    ...
    try
      ...
      while not mtCor.Eof do begin
        tbCor.Insert;
        tbCorTipoAcesso.Value := TipoAcesso;
...
begin
  ...
  try
    ...
    while not mtEtapas.Eof do begin
      ...
      tbEtapas.Insert;
      TransfDados(mtEtapas, tbEtapas);
      tbEtapasTempo.Value := tbEtapasTempo.Value - TempoAnt;
      tbEtapasValor.Value := tbEtapasValor.Value - ValorAnt;
