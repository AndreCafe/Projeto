...
uses
  ...
  cxClasses, cxMemo, cxLookAndFeelPainters;
...
begin
  if (edNome.EditValue=Null) or (Trim(edNome.EditValue)='') then
    raise ENexCafe.Create('É necessário informar o nome do patrocinador');

  if (edURL1.Text='') or (Trim(edURL1.Text)='') then
    raise ENexCafe.Create('É necessário o endereço da página principal a ser exibida');
