...
type  
  ...
  protected  
    ...
    function GetHoraTarifa: PncHoraTarifa;
  ...
  public
    ...
    property HoraTarifa : PncHoraTarifa
      read GetHoraTarifa;
...
begin
  ...
  if (FTarifaA<>nil) and (FTarifaAI=HoraTarifa^[FDia, FHora]) then
    FTarifa := FTarifaA
    ...
    FTarifaAI := HoraTarifa^[FDia, FHora];
    FTarifa := gTarifas.PorCor[IDTipoAcesso, FTarifaAI];
    ...
    while (FTarifa=nil) and (Avancou<24) do begin
      ...
      FTarifaAI := HoraTarifa^[aDia, aHora];
      FTarifa := gTarifas.PorCor[IDTipoAcesso, FTarifaAI];
      Inc(Avancou);
    end;
  ...
  if (FTarifa<>FTarifaA) then begin
    ...
    aTempoCorrido := aTempoCorrido - FTicksTarifa;
//    aTempoCorrido := aTempoCalc - FTicksTarifa;
    
    if (aTempoCorrido>0) and (FTarifaA<>nil) and (FTarifaA.Reinicia) then
      { Só nos interessa o Tempo Corrido desde o ultimo inicio de 1a ETAPA da Cor Anterior }
      aTempoCorrido := aTempoCorrido mod FTarifaA.TempoTotal;

    { Obtem o indice da etapa de preço atual }
    FEtapa := FTarifa.ObtemIndProxPreco(aTempoCorrido);
      
    FTicksTarifa := aTempoCorrido + FTicksTarifa;
    FTicksPreco := FTicksTarifa;
  end else
  if ((aTempoCorrido-FTicksPreco) >= FTarifa.PPrecos^[FEtapa].etTempo) then begin
    ...
    if (Length(FTarifa.PPrecos^)>0) and FTarifa.Reinicia then begin
      FEtapa := 0;
      FTicksPreco := aTempoCorrido;
    end;
  end;
...
begin
  if FTipoAcessoObj<>nil then
    Result := FTipoAcessoObj.PHoraTarifa else
    Result := @HoraTarBranco;
end;
...
begin
  ...
  FTicksResPass := 0;
  FNumTicks := 0;
  FCredTotalG := FPassaportes.TempoTotalDisp(FAgora) + FCredito.Ticks + FCredValorT;
  try
...
begin
  Result := FormatDateTime('hh:mm:ss', Cronometro.DateTime);
end;
