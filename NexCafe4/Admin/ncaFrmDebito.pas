...
uses
  ...
  cxImageComboBox, Buttons, PngSpeedButton, ncClassesBase, LMDControl;
...
begin
  ...
  if vgTPago.Properties.Value > vgTTotalF.Properties.Value then
    raise ENexCafe.Create('Valor pago não pode ser maior que total final');

  if vgTDEsconto.Properties.Value > vgTSel.Properties.Value then
    raise ENexCafe.Create('Valor do desconto não pode ser maior que o valor a pagar'); 
...
begin
  ...
  case idTipoItem of
    itSessao : begin
      if tbSessao.Locate('ID', idItemID, []) then begin
        Values[aItem, 1] := 'Acesso Máq: '+tbSessaoMaq.AsString;
        Values[aItem, 2] := tbSessaoTermino.Value;
      end else
        Values[aItem, 1] := 'Acesso';
    end;
    
    itMovEst : begin
      Values[aItem, 1] := 'Venda';
      if not tbMovEst.Locate('ID', idItemID, []) then Exit;
      Values[aItem, 2] := tbMovEstDataHora.Value;
      if not tbPro.Locate('ID', tbMovEstProduto.Value, []) then Exit;
      Values[aItem, 1] := 'Venda: ' + tbMovEstQuant.AsString + '-' + tbProDescricao.Value;
    end;

    itImpressao : begin
      Values[aItem, 1] := 'Impressão';
      if tbImp.Locate('ID', idItemID, []) then
        Values[aItem, 2] := tbImpDataHora.Value;
    end;

    itTempo : begin
      Values[aItem, 1] := 'Tempo';
      if not tbTempo.Locate('ID', idItemID, []) then Exit;
      Values[aItem, 2] := tbTempoDataHora.Value;
      if not tbTran.Locate('ID', tbTempoTran.Value, []) then Exit;
      Values[aItem, 1] := 'Tempo: '+tbTranDescr.Value
    end;
  end;
end;
