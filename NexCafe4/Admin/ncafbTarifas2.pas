...
uses
  ...
  cxGridLevel, cxGrid, cxGroupBox, ncafbPacotes;
...
uses ncaFrmTarifa, ncClassesBase, ncaFrmPri, ncaDM, ncIDRecursos;
...
begin
  ...
  if cbGrupo.Checked then begin
    ...
    if SimNao('Confirma a Desativação do Grupo de Tarifas ?') then begin
      gConfig.AtualizaCache;
...
begin
  ...
  if cbHorario.Checked then begin
    ...
    if SimNao('Confirma a Desativação de Tarifas por Dia e Horário?') then begin
      gConfig.AtualizaCache;
