...
uses ufmImagens, ncaDM, ncaFrmPri, ncIDRecursos,
  ...
  ncVersoes, ncImpressao, ncaFrmImp, ncFrmSuporteRem, ncaFrmMaq;
...
begin
  if Maquinas[aRecord.RecordIndex]=nil then exit;
  AText := Maquinas[ARecord.RecordIndex].Nome;
end;
...
begin
  with Dados, CM do begin
    ...
    if M<>nil then
      cmBloqSite.Caption := '&P. Bloquear site: ' + M.SiteAtual else
...
begin
  ...
  if TFrmTempo.Create(Self).Editar(True, True, @DT, 0, (Sender=cmVenderCred), MinutosT, ValorT) then  
    try
