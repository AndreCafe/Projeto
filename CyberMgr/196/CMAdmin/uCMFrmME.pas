...
type
  ...
  TFrmME = class(TForm)
    ...
    InspCab1Cliente: TdxInspectorDBExtLookupRow;
...
begin
  ...
  if tTranTipo.Value = ttEstVenda then
...
begin
  ...
  try
    ...
    if tTranNomeContato.Value>'' then 
      Writeln(Arq, ' CLIENTE: ' + tTranNomeContato.Value);
    ...
    Writeln(Arq, CharStr('-', tbConfigLargImpRec.Value));
    if tTranDescontoProduto.Value > 0.009 then begin
      Writeln(Arq, ' SUB-TOTAL === ' + ValorStr(tTranProdutos.Value));
      Writeln(Arq, ' DESCONTO ==== ' + ValorStr(tTranDescontoProduto.Value));
    end;    
    Writeln(Arq,   ' TOTAL FINAL = ' + ValorStr(tTranTotalFinal.Value));
    Writeln(Arq, CharStr('-', tbConfigLargImpRec.Value));
