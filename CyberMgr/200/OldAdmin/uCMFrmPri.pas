...
uses
  ...
  LMDFormTabControl, dxPageControl, LMDFormDisplay,
  LMDContainerComponent,
  LMDBaseDialog, LMDAboutDlg, LMDVersionInfo, ImgList, lmdmsg,
  LMDWindowListDlg, dxPSCompsProvider, dxPSFillPatterns, dxPSEdgePatterns;
...
type
  ...
  TFrmPri = class(TForm)
    ...
    VI: TLMDVersionInfo;
    Icones16: TImageList;
    msgBox: TLMDMessageBoxDlg;
    cmTotPer: TdxBarButton;
    LMDSimplePanel2: TLMDSimplePanel;
    cmCadastrarPass: TdxBarButton;
    cmVenderPassaporte: TdxBarButton;
    dxBarButton1: TdxBarButton;
    cmAlterarSenha: TdxBarButton;
    cmImprimirPrePago: TdxBarButton;
    cmVendePreImp: TdxBarButton;
    ...
    procedure cmTotPerClick(Sender: TObject);
    procedure cmCadastrarPassClick(Sender: TObject);
    procedure cmVenderPassaporteClick(Sender: TObject);
    procedure dxBarButton1Click(Sender: TObject);
    procedure cmAlterarSenhaClick(Sender: TObject);
    procedure cmImprimirPrePagoClick(Sender: TObject);
    procedure cmVendePreImpClick(Sender: TObject);
  ...
  public
    ...
    procedure EditarME(Num: Integer);
...
uses 
  ...
  cmIDRecursos,
  dxGridMenus, 
  uCMFrmVendaPac, uCMFrmCaixa, uCMFrmLancExtra,
  uCMfbgTipoTarifas, uCMFrmHoraCor, uCMFrmContato, 
  uCMFrmPassaporte, uCMfbgTipoPass, uCMFrmVendaPass, uCMFrmHistorico,
  uCMFrmAlteraSenha, uCMFrmImpPrePagoValido, uCMfbgPassaportes;
...
procedure TFrmPri.EditarME(Num: Integer);
begin
  TFrmME.Create(Self).Editar(Num);
end;
...
begin
  case V of
    ...
    viAce : FrmAcessos := TfbgTran.Create(Self);
...
begin
  cmTotPer.Enabled := Permitido(daCaiVerAntesOutros);
  cmImprimirPrePago.Enabled := Permitido(daPPGImpPPInvalido) or Permitido(daPPGImpPPValido);
  if NumAberto > 0 then begin
    cmCaixa.Caption := 'Caixa';
    cmLancarEntrada.Enabled := Permitido(daCaiSuprimento);
    cmLancarSaida.Enabled := Permitido(daCaiSangria);
    cmListaCaixas.Enabled := Permitido(daCaiVerAntes) or Permitido(daCaiVerAntesOutros);
    cmVendaAvulsa.Enabled := True;
    cmVenderPac.Enabled := True;
    cmTempoAvulso.Enabled := True;
    cmCaixa.Enabled := Permitido(daCaiAbrirFechar) or Permitido(daCaiVerAtual);
  end else begin
...
begin
  TFrmLancExtra.Create(Self).Novo(FrmAcessos.Tab, ttSuprimentoCaixa);
end;

procedure TFrmPri.cmLancarSaidaClick(Sender: TObject);
begin
  TFrmLancExtra.Create(Self).Novo(FrmAcessos.Tab, ttSangriaCaixa);
end;

procedure TFrmPri.cmVendaAvulsaClick(Sender: TObject);
begin
  TFrmME.Create(Self).Criar(ttEstVenda);
end;
...
begin
  cmLog.Enabled := Dados.CM.UA.Admin;
  cmUsuarios.Enabled := Dados.CM.UA.Admin;
  cmCadastroPac.Enabled := Permitido(daCfgPrecos);
  cmEstoqueCompra.Enabled := Permitido(daEstCompras);
  cmAjustaSaida.Enabled := Permitido(daEstSaida);
  cmAjustaEntrada.Enabled := Permitido(daEstEntrada);
  cmLancarEntrada.Enabled := Permitido(daCaiSuprimento);
  cmLancarSaida.Enabled := Permitido(daCaiSangria);
  cmConfig.Enabled := Permitido(daCfgParametros);
  cmPrecos.Enabled := Permitido(daCfgPrecos);
end;
...
procedure TFrmPri.cmTotPerClick(Sender: TObject);
begin
  TFrmCaixa.Create(Self).CaixaPeriodo;
end;

procedure TFrmPri.cmCadastrarPassClick(Sender: TObject);
begin
  with TfbgTipoPass.Create(Self) do begin
    Inicializar(False, FiltroNenhum, 0);
    ShowModal;
    Free;
  end;
...
procedure TFrmPri.cmVenderPassaporteClick(Sender: TObject);
begin
  TFrmVendaPass.Create(Self).Novo(Dados.tbAcesso, False);
end;

procedure TFrmPri.dxBarButton1Click(Sender: TObject);
begin
  TFrmHistVer.Create(nil).ShowModal;
end;
...
procedure TFrmPri.cmImprimirPrePagoClick(Sender: TObject);
begin
  TFrmImpPrePagoValido.Create(Self).ShowModal;
end;

procedure TFrmPri.cmVendePreImpClick(Sender: TObject);
begin
  TFrmVendaPass.Create(Self).Novo(Dados.tbAcesso, True);
end;

procedure TFrmPri.dxBarButton2Click(Sender: TObject);
begin
  with TfbgPassaportes.Create(Self) do begin
    Inicializar(False, 0, 0);
    ShowModal;
    Free;
  end;  
