...
begin
  Result := False;
  try
    GLog.Log(Self,[lcTrace],'Processar '+FArq);
    if not FileExists(FArq) Then begin
      GLog.Log(Self,[lcTrace],'Processar - Arquivo nao encontrado'+FArq);
      Exit;
    end;
    FPDFDoc.FileName := FArq;
    maxpages := FPDFDoc.PageCount;
    GLog.Log(Self,[lcTrace],'PageCount: '+inttostr(maxpages));
    PrepareDirDest;
    maxp := 0;
    ...
    for i:=1 to maxpages do begin
      ...
      try
        GLog.Log(Self,[lcTrace],'Processar 1');
        bmp := FPDFDoc.GetBitmap(i, 72, 72);
        GLog.Log(Self,[lcTrace],'Processar 2');
        ...
        if maxp>0 then begin
          X := i mod maxp;
          if x=0 then x := maxp;
          bmp := FPDFDoc.GetBitmap(x, 72, 72);
        end;
      end;
  
      if BMP = nil then begin
        GLog.Log(Self,[lcTrace],'Falha no GetBitmap');
        Break;
      end;
  
      GLog.Log(Self,[lcTrace],'Processar 3');
      ...
      try
        GLog.Log(Self,[lcTrace],'Processar 4');
        ...
        GLog.Log(Self,[lcTrace],'Processar 5');
        ...
        if I=1 then begin
          if (FViewerHWND<>0) and (not FAdmin) then begin
            ...
            try
              ...
              sl.Values['ThumbW'] := IntToStr(kThumbWidth);
              sl.Values['ThumbH'] := IntToStr(thumb.Height);
        ...
        GLog.Log(Self, [lcTrace],'Processar - Salvou Pagina '+inttostr(i));
      finally
        jpg.free;
        thumb.free;
        bmp.free;
    ...
    GLog.Log(Self, [lcTrace],'Processar - FIM'+inttostr(i));
  finally
    if not Result then 
...
begin
  GLog.Log(Self, [lcTrace],'Execute Start');
  ...
  GLog.Log(Self, [lcTrace],'Execute End');
end;
