...
uses
  ...
  StShlCtl, StdCtrls, ComCtrls, STSHRTCT;
  ...
  if (FStatus > stConectado) then begin
    ...
    if (Maq<>nil) and ((Maq.Acesso>0) or Maq.Manutencao) and  (not Maq.AguardaPagto) then begin
      ...
      if Maq.Manutencao then begin
        ...
        if FFecharExp then FechaExplorer(False);
...
{        tmPiscaTempo.Enabled := (not FDesligaAviso) and
        ...
        begin
          ...
          Status := stCliPausa;
          if CM.Config.EncerramentoPrePago=eppEncerrarAcesso then
            CM.LogoutMaq(Maq.Numero);
        end else
...
begin
  ...
  if StringToBool(slRestr.Values['ReiniciarMaq']) then begin
    CMShutDown(nil, 1);
    Exit;
  end;
